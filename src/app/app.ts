...
import {
  ClosureDashboardComponent,
  ClosureStatusUpdateResult,
} from './closure-dashboard/closure-dashboard.component';
    ...
    <main>
      <header class="hero">
        ...
        <div class="view-switcher no-print">
          <button
            type="button"
            [class.active-view]="viewMode === 'schedule'"
          ...
          <button
            type="button"
            [class.active-view]="viewMode === 'meal-prep'"
            (click)="setViewMode('meal-prep')"
          >
            🍳 备餐产能与出餐核对
          </button>
          <button
            type="button"
            [class.active-view]="viewMode === 'volunteer-delivery'"
            (click)="setViewMode('volunteer-delivery')"
          >
            🚴 志愿者配送
          </button>
          <button
            type="button"
            [class.active-view]="viewMode === 'closure-dashboard'"
            (click)="setViewMode('closure-dashboard')"
          >
            📊 闭环仪表盘
          </button>
        </div>
        ...
        <div class="hero-actions no-print">
          <button
            type="button"
            class="ghost hero-kitchen-btn"
            (click)="quickOpenKitchenPrint()"
            *ngIf="viewMode === 'schedule' || viewMode === 'meal-prep'"
          >
            🖨️ 厨房批次打印
          </button>
          <button
            type="button"
            class="ghost hero-kitchen-btn temp-change-manage-btn"
            (click)="openTempChangeManagePanel()"
            *ngIf="viewMode === 'schedule'"
          >
            📋 临时变更管理
            <span class="tc-manage-count" *ngIf="getTempChangeCountByStatus().today > 0">{{
              getTempChangeCountByStatus().today
            }}</span>
          </button>
          <button type="button" class="ghost import-export-btn" (click)="openImportExportPanel()">
            📦 数据导入导出
          </button>
        </div>
      </header>
        ...
        <section class="layout">
          <aside class="stack">
            <form class="panel" (ngSubmit)="addElder()">
              ...
              <input
                name="elderPreference"
                [(ngModel)]="elderForm.preference"
                placeholder="餐食偏好（文字备注）"
              />
              <div class="tag-select">
                ...
                <div class="tag-select-grid">
                  <label class="tag-check" *ngFor="let tag of mealTags">
                    <input
                      type="checkbox"
                      [checked]="elderForm.mealTags.includes(tag.id)"
                      (change)="toggleElderTag(tag.id)"
                    />
                    <span>{{ tag.name }}</span>
                  </label>
                </div>
            ...
            <section class="panel elder-list-panel">
              <h2>
                老人列表 <span class="muted sm-label">({{ elders.length }}位)</span>
              </h2>
              <div class="elder-list">
                <ng-container *ngFor="let elder of elders">
                  <div
                    class="elder-card"
                    *ngIf="editingElderId !== elder.id"
                    (click)="selectElder(elder.id)"
                    [class.active]="selectedElderId === elder.id"
                  >
                    <div class="elder-card-header">
                      ...
                      <div class="elder-card-btns">
                        <button
                          type="button"
                          class="ghost sm visit-btn"
                          (click)="$event.stopPropagation(); startEditElder(elder)"
                        >
                          编辑
                        </button>
                        <button
                          type="button"
                          class="ghost sm visit-btn"
                          (click)="$event.stopPropagation(); openTempChangePanel(elder.id)"
                        >
                          临时变更
                        </button>
                        <button
                          type="button"
                          class="ghost sm visit-btn"
                          (click)="$event.stopPropagation(); openVisitPanel(elder.id)"
                        >
                          回访
                        </button>
                      </div>
                    ...
                    <div class="tag-row" *ngIf="elderMealTags(elder.id).length > 0">
                      <span
                        class="tag-chip sm"
                        *ngFor="let tag of elderMealTags(elder.id)"
                    ...
                    <div
                      class="temp-change-indicator"
                      *ngIf="getElderTempChanges(elder.id).length > 0"
                    >
                      <span
                        class="temp-change-badge"
                        *ngFor="let tc of getElderTempChanges(elder.id)"
                        >📋 {{ tc.date }} {{ tempChangeSummary(elder.id, tc.date) }}</span
                      >
                    </div>
                    <div class="last-visit" *ngIf="getLastVisit(elder.id)">
                      <span class="visit-dot"></span>
                      <span
                        >上次回访：{{ getLastVisit(elder.id)!.visitDate }} ·
                        {{ getLastVisit(elder.id)!.visitMethod }}</span
                      >
                  ...
                  <form
                    class="elder-card elder-edit-card"
                    *ngIf="editingElderId === elder.id"
                    (ngSubmit)="saveEditElder()"
                  ...
                  >
                    ...
                    <input
                      [(ngModel)]="elderEditForm.name"
                      name="editElderName"
                      placeholder="姓名"
                      required
                    />
                    <input
                      [(ngModel)]="elderEditForm.preference"
                      name="editElderPreference"
                      placeholder="餐食偏好（文字备注）"
                    />
                    <div class="tag-select">
                      <label class="tag-select-label">餐食标签</label>
                      <div class="tag-select-grid">
                        <label class="tag-check" *ngFor="let tag of mealTags">
                          <input
                            type="checkbox"
                            [checked]="elderEditForm.mealTags.includes(tag.id)"
                            (change)="toggleElderEditTag(tag.id)"
                          />
                          <span>{{ tag.name }}</span>
                        </label>
                      </div>
                    ...
                    <input
                      [(ngModel)]="elderEditForm.address"
                      name="editElderAddress"
                      placeholder="送餐地址"
                    />
                    <input
                      [(ngModel)]="elderEditForm.contact"
                      name="editElderContact"
                      placeholder="紧急联系"
                    />
                    <input
                      [(ngModel)]="elderEditForm.note"
                      name="editElderNote"
                      placeholder="备注"
                    />
                ...
                <div class="tag-item" *ngFor="let tag of mealTags">
                  <ng-container *ngIf="editingTagId !== tag.id">
                    <span
                      ...
                      >{{ tag.name }}</span
                    >
                    <button type="button" class="ghost sm tag-edit" (click)="startEditTag(tag)">
                      编辑
                    </button>
                    <button
                      type="button"
                      class="ghost sm tag-del"
                      (click)="removeMealTag(tag.id)"
                      *ngIf="tag.id.startsWith('custom-')"
                    >
                      ×
                    </button>
                  </ng-container>
            ...
            <form class="panel" (ngSubmit)="addVolunteer()">
              ...
              <input
                ...
                type="number"
                min="1"
                [(ngModel)]="volunteerForm.capacity"
                placeholder="每日可送数量"
              />
              <button>保存志愿者</button>
            </form>
          ...
          <section class="panel simulation-panel" *ngIf="simulationMode === 'active'">
            <div class="simulation-header">
              <div class="simulation-title">
                ...
                <h2>多日排班模拟</h2>
                <span class="sim-date-range"
                  >{{ simulationData?.startDate }} 至 {{ simulationData?.endDate }} ({{
                    getSimulationDatesCount()
                  }}天)</span
                >
              </div>
              <div class="simulation-actions">
                <button type="button" class="ghost sm" (click)="openSimulationPanel()">
                  📊 查看详情
                </button>
                <button
                  type="button"
                  class="auto-assign-btn sm"
                  (click)="autoAssignSimulationTasks()"
                >
                  🔄 自动分配
                </button>
                <button type="button" class="sm submit-btn" (click)="submitSimulation()">
                  📝 预览差异并提交
                </button>
                <button type="button" class="ghost sm cancel-btn" (click)="cancelSimulation()">
                  ✕ 取消模拟
                </button>
              </div>
            </div>
            <div class="simulation-day-tabs">
              ...
              >
                ...
                <span class="sim-day-count">
                  {{ getSimulationDayStats(date)?.assignedCount || 0 }}/{{
                    getSimulationDayStats(date)?.totalTasks || 0
                  }}
                </span>
              </button>
            </div>
          ...
          <section class="panel" *ngIf="simulationMode === 'off'">
            <div class="toolbar">
              ...
              <div class="sim-setup">
                ...
                <button type="button" class="auto-assign-btn" (click)="startSimulation()">
                  开始模拟
                </button>
              </div>
            </div>
            <p class="muted sim-desc">
              在模拟模式下，您可以先生成多日排班方案进行预览和调整，确认无误后再提交写入正式数据。模拟期间的所有操作都不会影响现有排班数据。
            </p>
          </section>
          ...
          <section class="panel">
            <div class="toolbar">
              <h2>
                {{
                  simulationMode === 'active' ? simulationViewDate + ' 模拟任务' : '每日送餐任务'
                }}
              </h2>
              <div>
                <input type="date" [(ngModel)]="taskDate" *ngIf="simulationMode === 'off'" />
                <button type="button" (click)="generateTasks()" *ngIf="simulationMode === 'off'">
                  生成当日任务
                </button>
                <button
                  type="button"
            ...
            <div class="auto-assign-result" *ngIf="autoAssignResult">
              <div class="auto-assign-header">
                <h3>自动分配结果</h3>
                <button type="button" class="ghost sm" (click)="autoAssignResult = null">
                  关闭
                </button>
              </div>
              <div class="auto-assign-summary">
                <span class="assign-ok">✓ 已分配 {{ autoAssignResult.assigned.length }} 单</span>
                <span class="assign-fail" *ngIf="autoAssignResult.failed.length > 0"
                  >✗ 未分配 {{ autoAssignResult.failed.length }} 单</span
                >
              </div>
              <div class="auto-assign-detail" *ngIf="autoAssignResult.assigned.length > 0">
                ...
                <div class="detail-row ok" *ngFor="let item of autoAssignResult.assigned">
                  <span class="detail-elder"
                    >{{ item.elderName }} <small>{{ item.elderAddress }}</small></span
                  >
            ...
            <div class="taskList">
              <article
                *ngFor="let task of filteredTasks()"
                [class.warn]="task.status === '异常'"
                [class.temp-change-card]="hasTempChangeOnDate(task.elderId, task.date)"
              >
                <div>
                  <strong
                    >{{ elderName(task.elderId) }}
                    <span
                      class="temp-change-badge"
                      *ngIf="hasTempChangeOnDate(task.elderId, task.date)"
                      >临时变更</span
                    ></strong
                  >
                  ...
                  <small
                    class="temp-change-detail"
                    *ngIf="hasTempChangeOnDate(task.elderId, task.date)"
                    >{{ tempChangeSummary(task.elderId, task.date) }}</small
                  >
                  <div class="tag-row" *ngIf="elderMealTags(task.elderId, task.date).length > 0">
                    <span
                      class="tag-chip"
                      *ngFor="let tag of elderMealTags(task.elderId, task.date)"
                      [style.background]="tag.color + '20'"
                      [style.color]="tag.color"
                      [style.borderColor]="tag.color + '50'"
                      >{{ tag.name }}</span
                ...
                <select
                  [ngModel]="task.volunteerId"
                  (ngModelChange)="assignTaskWithConflictCheck(task.id, $event)"
                >
                  <option value="">未分配</option>
                  <option *ngFor="let volunteer of volunteers" [value]="volunteer.id">
                    {{ volunteer.name }} · {{ volunteer.area }}
                  </option>
                </select>
                <div class="actions">
                  <button type="button" (click)="setStatusWithConflictCheck(task.id, '配送中')">
                    配送中
                  </button>
                  <button type="button" (click)="setStatusWithConflictCheck(task.id, '已送达')">
                    已送达
                  </button>
                  <button type="button" class="ghost" (click)="recordException(task.id)">
                    异常
                  </button>
                </div>
                <p>
                  {{ task.status }} <span *ngIf="task.exception">· {{ task.exception }}</span>
                </p>
              </article>
            </div>
          </section>
          ...
          <aside class="stack">
            <section class="panel">
              ...
              <div class="progress">
                <p>
                  <strong>{{ countByStatus('待分配') }}</strong
                  ><span>待分配</span>
                </p>
                <p>
                  <strong>{{ countByStatus('配送中') }}</strong
                  ><span>配送中</span>
                </p>
                <p>
                  <strong>{{ countByStatus('已送达') }}</strong
                  ><span>已送达</span>
                </p>
                <p>
                  <strong>{{ countByStatus('异常') }}</strong
                  ><span>异常</span>
                </p>
              </div>
            </section>
            ...
            <section class="panel">
              ...
              <div class="tag-stats">
                <div class="tag-stat-row" *ngFor="let stat of todayTagStats()">
                  <span
                    class="tag-chip"
                    [style.background]="stat.tag.color + '20'"
                    [style.color]="stat.tag.color"
                    [style.borderColor]="stat.tag.color + '50'"
                    >{{ stat.tag.name }}</span
                  >
            ...
            <section class="panel">
              <div class="toolbar" style="margin-bottom:12px">
                <h2>异常处置闭环</h2>
                <button type="button" class="ghost sm" (click)="openExceptionHistory()">
                  历史查询
                </button>
              </div>
              <div class="exc-status-row">
                <div class="exc-status-item">
                  <strong>{{ exceptionCountByStatus('待处理') }}</strong
                  ><span>待处理</span>
                </div>
                <div class="exc-status-item">
                  <strong>{{ exceptionCountByStatus('处理中') }}</strong
                  ><span>处理中</span>
                </div>
                <div class="exc-status-item">
                  <strong>{{ exceptionCountByStatus('已解决') }}</strong
                  ><span>已解决</span>
                </div>
              </div>
              <div class="exc-list">
                <div class="exc-item" *ngFor="let exc of todayUnresolvedExceptions()">
                  <div class="exc-item-header">
                    ...
                    <span
                      ...
                      [style.borderColor]="severityColor(exc.severity)"
                      >{{ exc.severity }}</span
                    >
                  </div>
                  <div class="exc-item-meta">
                    ...
                    <span
                      class="exc-status-tag"
                      [style.color]="statusColor(exc.status)"
                      [style.borderColor]="statusColor(exc.status)"
                      >{{ exc.status }}</span
                    >
                  </div>
                  <p class="exc-item-desc">{{ exc.description }}</p>
                  <div class="exc-item-handler" *ngIf="exc.handler">
                    <span>负责人：{{ exc.handler }}</span>
                  </div>
                  <div class="exc-item-actions">
                    <button
                      ...
                      class="ghost sm"
                      *ngIf="exc.status === '待处理'"
                      (click)="updateExceptionStatus(exc.id, '处理中')"
                    >
                      开始处理
                    </button>
                    <button
                      ...
                      class="sm"
                      *ngIf="exc.status === '处理中'"
                      (click)="updateExceptionStatus(exc.id, '已解决')"
                    >
                      标记已解决
                    </button>
                    <button
                      ...
                      class="ghost sm"
                      *ngIf="exc.status === '处理中'"
                      (click)="updateExceptionStatus(exc.id, '待处理')"
                    >
                      退回待处理
                    </button>
                  </div>
                </div>
                <p class="muted" *ngIf="todayUnresolvedExceptions().length === 0">
                  当日暂无待处理异常
                </p>
              </div>
            </section>
            ...
            <section class="panel">
              ...
              <div class="load-item" *ngFor="let volunteer of volunteers">
                ...
                <div class="load-bar-bg">
                  <div
                    class="load-bar-fill"
                    [style.width.%]="(assignedCount(volunteer.id) / volunteer.capacity) * 100"
                    [class.full]="assignedCount(volunteer.id) >= volunteer.capacity"
                    [class.near]="
                      assignedCount(volunteer.id) >= volunteer.capacity * 0.75 &&
                      assignedCount(volunteer.id) < volunteer.capacity
                    "
                  ></div>
                </div>
            ...
            <section class="panel">
              <div class="toolbar" style="margin-bottom:12px">
                <h2>☎️ 电话通知 & 回拨</h2>
                <button type="button" class="ghost sm" (click)="openPhoneNotificationPanel()">
                  管理
                </button>
              </div>
              <div class="phone-status-row">
                <div class="phone-status-item">
                  <strong>{{ getPendingNotificationCount() }}</strong
                  ><span>待通知</span>
                </div>
                <div class="phone-status-item">
                  <strong>{{ getCallbackRequiredNotificationCount() }}</strong
                  ><span>需回拨</span>
                </div>
                <div class="phone-status-item">
                  <strong>{{ getPendingCallbackCount() }}</strong
                  ><span>回拨中</span>
                </div>
              </div>
              <div class="phone-callback-list">
                <div class="phone-callback-item" *ngFor="let cb of activeCallbackTasksPreview()">
                  <div class="cb-item-header">
                    ...
                    <span
                      class="cb-status-tag"
                      [style.color]="cb.status === '待回拨' ? '#d9a84a' : '#5a8fd9'"
                      [style.borderColor]="cb.status === '待回拨' ? '#d9a84a' : '#5a8fd9'"
                      >{{ cb.status }}</span
                    >
                  </div>
                  ...
                  <div class="cb-item-actions">
                    <button type="button" class="ghost sm" (click)="editCallbackTask(cb.id)">
                      详情
                    </button>
                    <button
                      type="button"
                    ...
                    <button
                      ...
                      class="sm"
                      *ngIf="cb.status === '回拨中'"
                      (click)="editCallbackTask(cb.id)"
                    >
                      完成回拨
                    </button>
                  </div>
                </div>
                <p class="muted center" *ngIf="getPendingCallbackCount() === 0">
                  暂无待处理回拨任务
                </p>
              </div>
            </section>
          ...
          <div class="kanban-grid">
              ...
              <div class="kanban-cards">
                ...
                >
                  <div class="kanban-card-info">
                    ...
                    <p class="kanban-status" [class.warn]="task.status === '异常'">
                      {{ task.status }}
                    </p>
                  </div>
                  <div class="kanban-order-btns">
                    <button
                      ...
                      class="ghost sm"
                      [disabled]="i === 0"
                      (click)="moveTask(group.volunteer.id, task.id, -1)"
                    >
                      ↑
                    </button>
                    <button
                      type="button"
                      class="ghost sm"
                      [disabled]="i === group.tasks.length - 1"
                      (click)="moveTask(group.volunteer.id, task.id, 1)"
                    >
                      ↓
                    </button>
            ...
            <div class="kanban-column kanban-unassigned">
              ...
              <div class="kanban-cards">
                <div
                  class="kanban-card"
                  *ngFor="let task of unassignedKanbanTasks()"
                  [class.temp-change-card]="hasTempChangeOnDate(task.elderId, task.date)"
                >
                  <div class="kanban-card-info">
                    <strong
      ...
      <div class="modal-overlay" *ngIf="visitPanelVisible" (click)="closeVisitPanel()">
        <div class="modal-panel" (click)="$event.stopPropagation()">
          ...
          <div class="modal-tabs">
            <button
              type="button"
              [class.active-tab]="visitTab === 'form'"
              (click)="visitTab = 'form'"
            >
              新增回访
            </button>
            <button
              type="button"
              [class.active-tab]="visitTab === 'history'"
              (click)="visitTab = 'history'"
            >
              历史记录
          ...
          <div class="modal-body">
            <form *ngIf="visitTab === 'form'" class="visit-form" (ngSubmit)="submitVisit()">
              ...
              <div class="form-row">
                ...
                <textarea
                  name="healthFeedback"
                  [(ngModel)]="visitForm.healthFeedback"
                  rows="3"
                  placeholder="如：精神状态良好，血压稳定，近期无不适..."
                ></textarea>
              </div>
              <div class="form-row">
                <label>用餐情况反馈</label>
                <textarea
                  name="mealFeedback"
                  [(ngModel)]="visitForm.mealFeedback"
                  rows="3"
                  placeholder="如：饭菜合口味，饭量正常，建议增加汤品..."
                ></textarea>
              </div>
              <div class="form-row">
                <label>下次关注事项</label>
                <textarea
                  name="nextAttention"
                  [(ngModel)]="visitForm.nextAttention"
                  rows="3"
                  placeholder="如：下周提醒复诊，关注血糖变化..."
                ></textarea>
              </div>
            ...
            <div *ngIf="visitTab === 'history'" class="visit-history">
              <div
                ...
                *ngFor="let record of getElderVisits(selectedElderIdForVisit)"
              >
                <div class="visit-history-header">
                  ...
                  <button type="button" class="ghost sm" (click)="deleteVisit(record.id)">
                    删除
                  </button>
                </div>
      ...
      <div class="modal-overlay" *ngIf="tempChangePanelVisible" (click)="closeTempChangePanel()">
        <div class="modal-panel temp-change-modal" (click)="$event.stopPropagation()">
          ...
          <div class="modal-body">
            <div
              *ngIf="getElderTempChanges(tempChangeFormElderId!).length > 0 && !editingTempChangeId"
              class="temp-change-existing"
            >
              <h3>已有临时变更</h3>
              <div
                class="temp-change-list-item"
                *ngFor="let tc of getElderTempChanges(tempChangeFormElderId!)"
              >
                <div class="temp-change-list-info">
                  <strong>{{ tc.date }}</strong>
                  <span class="temp-change-list-summary">{{
                    tempChangeSummary(tc.elderId, tc.date)
                  }}</span>
                ...
                <div class="temp-change-list-actions">
                  ...
                  <button type="button" class="ghost sm" (click)="deleteTempChange(tc.id)">
                    删除
                  </button>
                </div>
              </div>
            ...
            <form class="temp-change-form" (ngSubmit)="saveTempChange()">
              ...
              <div class="form-row">
                ...
                <input
                  name="tempChangeReason"
                  [(ngModel)]="tempChangeForm.reason"
                  placeholder="如：老人临时住女儿家"
                  required
                />
              </div>
              <div class="form-row">
                <label>临时送餐地址</label>
                <input
                  name="tempChangeAddress"
                  [(ngModel)]="tempChangeForm.address"
                  placeholder="留空则使用长期档案地址"
                />
              </div>
              <div class="form-row">
                <label>临时联系方式</label>
                <input
                  name="tempChangeContact"
                  [(ngModel)]="tempChangeForm.contact"
                  placeholder="留空则使用长期档案联系方式"
                />
              </div>
              <div class="form-row">
                ...
                <div class="tag-select-grid">
                  <label class="tag-check" *ngFor="let tag of mealTags">
                    <input
                      type="checkbox"
                      [checked]="(tempChangeForm.mealTagIds || []).includes(tag.id)"
                      (change)="toggleTempChangeFormTag(tag.id)"
                    />
                    <span>{{ tag.name }}</span>
                  </label>
                </div>
              ...
              <div class="form-row">
                ...
                <input
                  name="tempChangeSpecialNote"
                  [(ngModel)]="tempChangeForm.specialMealNote"
                  placeholder="留空则使用长期档案备注"
                />
              </div>
              <div class="form-row">
                <label>指定志愿者</label>
                ...
                <div class="conflict-resolution">
                  <label>
                    <input
                      ...
                      [(ngModel)]="tempChangeConflictResolution"
                      value="overwrite-task"
                    />
                    <span>覆盖任务变更</span>
                  </label>
                  <label>
              ...
              <div class="form-actions">
                <button
                  ...
                  class="ghost"
                  (click)="
                    editingTempChangeId = null;
                    tempChangeForm = { date: todayStr, reason: '' };
                    tempChangeConflictInfo = null
                  "
                  *ngIf="editingTempChangeId"
                >
                  取消编辑
                </button>
                <button type="submit">
                  {{ editingTempChangeId ? '保存修改' : '添加临时变更' }}
                </button>
              </div>
            </form>
          </div>
      ...
      <div
        class="modal-overlay"
        *ngIf="tempChangeManagePanelVisible"
        (click)="closeTempChangeManagePanel()"
      >
        <div class="modal-panel temp-change-manage-modal" (click)="$event.stopPropagation()">
          <div class="modal-header">
            ...
            <button type="button" class="ghost sm" (click)="closeTempChangeManagePanel()">
              关闭
            </button>
          </div>
          ...
          <div class="tc-manage-summary-bar">
            ...
            <div class="tc-stat-item">
              <span class="tc-stat-num upcoming-num">{{
                getTempChangeCountByStatus().upcoming
              }}</span>
              <span class="tc-stat-label">即将生效</span>
            </div>
            <div class="tc-stat-item">
              <span class="tc-stat-num expired-num">{{
                getTempChangeCountByStatus().expired
              }}</span>
            ...
            <div class="tc-manage-header-actions">
              <button
                type="button"
                class="ghost sm"
                [disabled]="getTempChangeCountByStatus().expired === 0"
                (click)="cleanupExpiredTempChanges()"
              >
                🗑️ 清理过期
              </button>
            </div>
          ...
          <div class="tc-filter-tabs">
            <button
              type="button"
              [class.active-filter]="tempChangeManageFilter === 'all'"
              (click)="setTempChangeManageFilter('all')"
            >
              全部 <small>({{ temporaryDeliveryChanges.length }})</small>
            </button>
            <button
              type="button"
              [class.active-filter]="tempChangeManageFilter === 'today'"
              (click)="setTempChangeManageFilter('today')"
            >
              🔴 今日 <small>({{ getTempChangeCountByStatus().today }})</small>
            </button>
            <button
              type="button"
              [class.active-filter]="tempChangeManageFilter === 'upcoming'"
              (click)="setTempChangeManageFilter('upcoming')"
            >
              🟡 即将生效 <small>({{ getTempChangeCountByStatus().upcoming }})</small>
            </button>
            <button
              type="button"
              [class.active-filter]="tempChangeManageFilter === 'expired'"
              (click)="setTempChangeManageFilter('expired')"
            >
          ...
          <div class="tc-manage-body">
            ...
            <div class="tc-date-group" *ngFor="let dateKey of getGroupedDateKeys()">
              <div class="tc-date-header">
                <h3>📅 {{ dateKey }}</h3>
                <span class="tc-date-count"
                  >{{ getTempChangesGroupedByDate().get(dateKey)!.length }}条变更</span
                >
              </div>
                ...
                >
                  <div class="tc-change-main" (click)="toggleTempChangeManageExpand(change.id)">
                    <div class="tc-change-info">
                      ...
                      <div class="tc-change-summary">
                        {{ tempChangeSummary(change.elderId, change.date) }}
                      </div>
                    ...
                    <div class="tc-change-actions">
                      ...
                      <span
                        class="tc-expand-arrow"
                        [class.rotated]="tempChangeManageExpandedId === change.id"
                        >▼</span
                      >
                    </div>
                  </div>
                    ...
                    <div class="tc-detail-grid">
                        ...
                        <div class="tc-detail-compare">
                          <div class="tc-compare-item original">
                            ...
                            <div class="tc-tag-row">
                              ...
                              <small
                                class="muted"
                                *ngIf="getTempChangeOriginalMealTags(change).length === 0"
                                >无</small
                              >
                            </div>
                          </div>
                          ...
                          <div class="tc-compare-item new" *ngIf="change.mealTagIds">
                            ...
                            <div class="tc-tag-row">
                              ...
                              <small
                                class="muted"
                                *ngIf="getTempChangeEffectiveMealTags(change).length === 0"
                                >清空所有标签</small
                              >
                            </div>
                          </div>
                      ...
                      <div class="tc-detail-col">
                        ...
                        <div class="tc-detail-value" [class.has-change]="change.volunteerId">
                          {{
                            change.volunteerId
                              ? '指定：' + getTempChangeVolunteerName(change)
                              : '不修改，保留任务中的志愿者分配'
                          }}
                        </div>
                      </div>
      ...
      <div class="modal-overlay" *ngIf="taskModConflictVisible" (click)="closeTaskModConflict()">
        <div class="modal-panel" style="max-width:520px" (click)="$event.stopPropagation()">
          <div class="modal-header">
            <div>
              <h2>⚠️ 临时变更冲突检测</h2>
              <p class="muted" *ngIf="taskModConflict">
                {{ elderName(taskModConflict.tempChange.elderId) }} ·
                {{ taskModConflict.tempChange.date }}
              </p>
            </div>
            ...
            <div class="temp-change-existing" *ngIf="taskModConflict">
              ...
              <div class="temp-change-list-item">
                <div class="temp-change-list-info">
                  ...
                  <span class="temp-change-list-summary">{{
                    tempChangeSummary(
                      taskModConflict.tempChange.elderId,
                      taskModConflict.tempChange.date
                    )
                  }}</span>
              ...
              <div class="conflict-resolution">
                <label *ngIf="taskModConflict.operation === 'assign-volunteer'">
                  <input
                    type="radio"
                    name="taskModResolution"
                    [(ngModel)]="taskModConflict.resolution"
                    value="override-temp"
                  />
                  <span
                    ><strong>同步修改临时变更</strong>：同时更新临时变更中的志愿者为当前选择</span
                  >
                </label>
                <label>
                  <input
      ...
      <div class="modal-overlay" *ngIf="exceptionPanelVisible" (click)="closeExceptionPanel()">
        <div class="modal-panel exc-modal" (click)="$event.stopPropagation()">
          ...
          <div class="modal-tabs">
            <button
              type="button"
              [class.active-tab]="exceptionPanelTab === 'form'"
              (click)="exceptionPanelTab = 'form'"
            >
              登记异常
            </button>
            <button
              type="button"
              [class.active-tab]="exceptionPanelTab === 'list'"
              (click)="exceptionPanelTab = 'list'"
            >
              异常列表
              <span class="badge" *ngIf="exceptionRecords.length > 0">{{
          ...
          <div class="modal-body">
            <form
              *ngIf="exceptionPanelTab === 'form'"
              class="exc-form"
              (ngSubmit)="submitException()"
            >
              <div class="form-row">
                <label>异常分类</label>
                <div class="method-group">
                  <label class="method-item" *ngFor="let cat of EXCEPTION_CATEGORIES">
                    <input
                      type="radio"
                      name="excCategory"
                      [(ngModel)]="exceptionForm.category"
                      [value]="cat"
                    />
                    <span>{{ cat }}</span>
                  </label>
                ...
                <div class="method-group">
                  <label class="method-item" *ngFor="let sev of EXCEPTION_SEVERITIES">
                    <input
                      type="radio"
                      name="excSeverity"
                      [(ngModel)]="exceptionForm.severity"
                      [value]="sev"
                    />
                    <span>{{ sev }}</span>
                  </label>
            ...
            <div *ngIf="exceptionPanelTab === 'list'" class="exc-modal-list">
              ...
              <div class="exc-modal-items">
                  ...
                  <div class="exc-modal-item-actions">
                    ...
                    <ng-container *ngIf="exc.status === '处理中' || exc.status === '已解决'">
                      ...
                      <button
                        ...
                        class="ghost sm"
                        (click)="
                          updateExceptionResult(exc.id, resultInput.value); resultInput.value = ''
                        "
                      >
                        保存结果
                      </button>
                ...
                <p class="muted center" *ngIf="filteredExceptionRecords().length === 0">
                  暂无匹配的异常记录
                </p>
              </div>
            </div>
        ...
        <div class="modal-panel exc-modal" (click)="$event.stopPropagation()">
          ...
          <div class="modal-body">
            ...
            <div class="exc-modal-items">
              ...
              <p class="muted center" *ngIf="historyExceptionRecords().length === 0">
                暂无匹配的历史异常记录
              </p>
            </div>
          </div>
      ...
      <div
        class="modal-overlay"
        *ngIf="phoneNotificationPanelVisible"
        (click)="closePhoneNotificationPanel()"
      >
        <div class="modal-panel phone-notif-modal" (click)="$event.stopPropagation()">
          <div class="modal-header">
            ...
            <button type="button" class="ghost sm" (click)="closePhoneNotificationPanel()">
              关闭
            </button>
          </div>
          ...
          <div class="modal-tabs">
            <button
              type="button"
              [class.active-tab]="phoneNotificationTab === 'list'"
              (click)="phoneNotificationTab = 'list'"
            >
              📋 通知清单
              <span class="badge" *ngIf="phoneNotifications.length > 0">{{
                phoneNotifications.length
              }}</span>
            </button>
            <button
              type="button"
              [class.active-tab]="phoneNotificationTab === 'callback'"
              (click)="phoneNotificationTab = 'callback'"
            >
              🔄 回拨任务
              <span class="badge" *ngIf="getPendingCallbackCount() > 0">{{
            ...
            <div *ngIf="phoneNotificationTab === 'list'" class="phone-notif-list">
              ...
              <div class="phone-notif-items">
                <div class="phone-notif-item" *ngFor="let notif of filteredPhoneNotifications()">
                  <div class="phone-notif-header">
                    ...
                    <div class="phone-notif-tags">
                      ...
                      <span
                        ...
                        [style.color]="notifStatusColor(notif.notificationStatus)"
                        [style.borderColor]="notifStatusColor(notif.notificationStatus)"
                        >{{ notif.notificationStatus }}</span
                      >
                    </div>
                  </div>
                  ...
                  <div
                    class="phone-notif-callback-info"
                    *ngIf="getTaskCallbacks(notif.taskId).length > 0"
                  >
                    <span class="cb-count"
                      >🔄 回拨记录：{{ getTaskCallbacks(notif.taskId).length }} 次</span
                    >
                    <span class="cb-latest" *ngIf="getTaskCallbacks(notif.taskId)[0]">
                      最近：{{
                        getTaskCallbacks(notif.taskId)[0].nextCallbackTime | date: 'MM-dd HH:mm'
                      }}
                  ...
                  <div class="phone-notif-actions">
                    <button
                      type="button"
                      class="ghost sm"
                      *ngIf="notif.notificationStatus !== '已通知'"
                      (click)="updateNotificationStatus(notif.id, '已通知')"
                    >
                      标记已通知
                    </button>
                    <button
                      type="button"
                      class="ghost sm"
                      *ngIf="notif.notificationStatus === '未通知'"
                      (click)="updateNotificationStatus(notif.id, '未接通')"
                    >
                      未接通
                    </button>
                    <button
                      type="button"
                      class="sm"
                      style="background:#5a8fd9"
                      (click)="openCallbackForm(notif)"
                    >
                ...
                <p class="muted center" *ngIf="filteredPhoneNotifications().length === 0">
                  暂无电话通知记录
                </p>
              </div>
            </div>
              ...
              <div class="callback-items">
                <div class="callback-item" *ngFor="let cb of filteredCallbackTasks()">
                  <div class="callback-item-header">
                    ...
                    <span
                      class="cb-status-badge"
                      [style.background]="cbStatusBgColor(cb.status)"
                      [style.color]="cb.status === '已完成' ? '#fff' : '#315448'"
                    >
                      {{ cb.status }}
                    </span>
                  </div>
                  ...
                  <div class="callback-item-actions">
                    ...
                    <button
                      type="button"
                      class="ghost sm tag-del"
                      (click)="deleteCallbackTask(cb.id)"
                    >
                      删除
                    </button>
                  </div>
                ...
                <p class="muted center" *ngIf="filteredCallbackTasks().length === 0">
                  暂无回拨任务
                </p>
              </div>
            </div>
          ...
          <div class="modal-header">
            <div>
              ...
              <p class="muted" *ngIf="selectedNotificationForCallback">
                {{ getNotificationElderName(selectedNotificationForCallback) }} ·
                {{ selectedNotificationForCallback.phone }}
              </p>
            </div>
            ...
            <form class="callback-form" (ngSubmit)="submitCallbackTask()">
              <div class="form-row">
                ...
                <input
                  ...
                  [(ngModel)]="callbackForm.nextCallbackTime"
                  required
                />
              </div>
              <div class="form-row">
      ...
      <div
        class="modal-overlay"
        *ngIf="importExportPanelVisible"
        (click)="closeImportExportPanel()"
      >
        <div class="modal-panel import-export-modal" (click)="$event.stopPropagation()">
          ...
          <div class="modal-tabs">
            <button
              type="button"
              [class.active-tab]="importTab === 'export'"
            ...
            <button
              type="button"
              [class.active-tab]="importTab === 'import'"
              (click)="importTab = 'import'; resetImport()"
            >
              📥 导入数据
            </button>
          </div>
          ...
          <div class="modal-body">
            <div *ngIf="importTab === 'export'" class="export-section">
              <div class="export-info">
                ...
                <ul class="export-list">
                  <li>
                    <strong>{{ elders.length }}</strong> 位老人档案
                  </li>
                  <li>
                    <strong>{{ volunteers.length }}</strong> 名志愿者
                  </li>
                  <li>
                    <strong>{{ tasks.length }}</strong> 条送餐任务
                  </li>
                  <li>
                    <strong>{{ mealTags.length }}</strong> 个餐食标签
                  </li>
                  <li>
                    <strong>{{ exceptionRecords.length }}</strong> 条异常记录
                  </li>
                  <li>
                    <strong>{{ visitRecords.length }}</strong> 条回访记录
                  </li>
                  <li>
                    <strong>{{ phoneNotifications.length }}</strong> 条电话通知
                  </li>
                  <li>
                    <strong>{{ callbackTasks.length }}</strong> 条回拨任务
                  </li>
                  <li>
                    <strong>{{ temporaryDeliveryChanges.length }}</strong> 条临时送餐变更
                  </li>
                </ul>
              </div>
            ...
            <div *ngIf="importTab === 'import'" class="import-section">
              <div
                *ngIf="!importPreview && !importError && !importSuccess"
              ...
              <div *ngIf="importPreview && importPreviewSummary" class="import-preview">
                <div class="preview-header">
                  <h3>导入预览</h3>
                  <p class="muted" *ngIf="importedData">
                    导出时间：{{ importedData.exportedAt | date: 'yyyy-MM-dd HH:mm' }}
                  </p>
                </div>
                ...
                <div class="preview-legend">
                  ...
                  <span class="legend-item"
                    ><span class="legend-dot duplicate"></span> 重复（无变化）</span
                  >
                </div>
                ...
                <div class="preview-cards">
                  <div class="preview-card" *ngIf="importPreviewSummary.elders.total > 0">
                    ...
                    <div class="preview-stats">
                      ...
                      <span class="stat overwrite"
                        >~{{ importPreviewSummary.elders.overwrite }}</span
                      >
                      <span class="stat duplicate"
                        >={{ importPreviewSummary.elders.duplicate }}</span
                      >
                    </div>
                  </div>
                  ...
                  <div class="preview-card" *ngIf="importPreviewSummary.volunteers.total > 0">
                    ...
                    <div class="preview-stats">
                      ...
                      <span class="stat overwrite"
                        >~{{ importPreviewSummary.volunteers.overwrite }}</span
                      >
                      <span class="stat duplicate"
                        >={{ importPreviewSummary.volunteers.duplicate }}</span
                      >
                    </div>
                  </div>
                  ...
                  <div class="preview-card" *ngIf="importPreviewSummary.tasks.total > 0">
                    ...
                    <div class="preview-stats">
                      ...
                      <span class="stat overwrite"
                        >~{{ importPreviewSummary.tasks.overwrite }}</span
                      >
                      <span class="stat duplicate"
                        >={{ importPreviewSummary.tasks.duplicate }}</span
                      >
                    </div>
                  </div>
                  ...
                  <div class="preview-card" *ngIf="importPreviewSummary.mealTags.total > 0">
                    ...
                    <div class="preview-stats">
                      ...
                      <span class="stat overwrite"
                        >~{{ importPreviewSummary.mealTags.overwrite }}</span
                      >
                      <span class="stat duplicate"
                        >={{ importPreviewSummary.mealTags.duplicate }}</span
                      >
                    </div>
                  </div>
                  ...
                  <div class="preview-card" *ngIf="importPreviewSummary.exceptionRecords.total > 0">
                    ...
                    <div class="preview-stats">
                      ...
                      <span class="stat overwrite"
                        >~{{ importPreviewSummary.exceptionRecords.overwrite }}</span
                      >
                      <span class="stat duplicate"
                        >={{ importPreviewSummary.exceptionRecords.duplicate }}</span
                      >
                    </div>
                  </div>
                  ...
                  <div class="preview-card" *ngIf="importPreviewSummary.visitRecords.total > 0">
                    ...
                    <div class="preview-stats">
                      ...
                      <span class="stat overwrite"
                        >~{{ importPreviewSummary.visitRecords.overwrite }}</span
                      >
                      <span class="stat duplicate"
                        >={{ importPreviewSummary.visitRecords.duplicate }}</span
                      >
                    </div>
                  </div>
                  ...
                  <div
                    class="preview-card"
                    *ngIf="importPreviewSummary.phoneNotifications.total > 0"
                  >
                    <h4>📞 电话通知</h4>
                    <div class="preview-stats">
                      <span class="stat new"
                        >+{{ importPreviewSummary.phoneNotifications.new }}</span
                      >
                      <span class="stat overwrite"
                        >~{{ importPreviewSummary.phoneNotifications.overwrite }}</span
                      >
                      <span class="stat duplicate"
                        >={{ importPreviewSummary.phoneNotifications.duplicate }}</span
                      >
                    </div>
                  </div>
                  ...
                  <div class="preview-card" *ngIf="importPreviewSummary.callbackTasks.total > 0">
                    ...
                    <div class="preview-stats">
                      ...
                      <span class="stat overwrite"
                        >~{{ importPreviewSummary.callbackTasks.overwrite }}</span
                      >
                      <span class="stat duplicate"
                        >={{ importPreviewSummary.callbackTasks.duplicate }}</span
                      >
                    </div>
                  </div>
                  ...
                  <div
                    class="preview-card"
                    *ngIf="importPreviewSummary.temporaryDeliveryChanges.total > 0"
                  >
                    <h4>📋 临时送餐变更</h4>
                    <div class="preview-stats">
                      <span class="stat new"
                        >+{{ importPreviewSummary.temporaryDeliveryChanges.new }}</span
                      >
                      <span class="stat overwrite"
                        >~{{ importPreviewSummary.temporaryDeliveryChanges.overwrite }}</span
                      >
                      <span class="stat duplicate"
                        >={{ importPreviewSummary.temporaryDeliveryChanges.duplicate }}</span
                      >
                    </div>
                    <div
                      class="preview-items-sample"
                      *ngIf="importPreview && importPreview.temporaryDeliveryChanges.length > 0"
                    >
                      <div
                        class="preview-item"
                        *ngFor="let item of importPreview.temporaryDeliveryChanges.slice(0, 5)"
                      ...
                      <p class="muted" *ngIf="importPreview.temporaryDeliveryChanges.length > 5">
                        ...还有 {{ importPreview.temporaryDeliveryChanges.length - 5 }} 条未显示
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div class="import-actions">
                  ...
                  <button type="button" class="confirm-btn" (click)="confirmImport()">
                    确认导入
                  </button>
                </div>
              </div>
        ...
        <div class="modal-panel simulation-modal" (click)="$event.stopPropagation()">
          <div class="modal-header">
            <div>
              <h2>排班模拟详情</h2>
              <p class="muted">
                {{ simulationData?.startDate }} 至 {{ simulationData?.endDate }} · 共
                {{ getSimulationDatesCount() }} 天
              </p>
            </div>
          ...
          <div class="modal-tabs">
            <button
              type="button"
              [class.active-tab]="simulationDetailTab === 'overview'"
              (click)="simulationDetailTab = 'overview'"
            >
              📊 总览
            </button>
            <button
              type="button"
              [class.active-tab]="simulationDetailTab === 'load'"
              (click)="simulationDetailTab = 'load'"
            >
              💪 志愿者负载
            </button>
            <button
              type="button"
              [class.active-tab]="simulationDetailTab === 'unassigned'"
              (click)="simulationDetailTab = 'unassigned'"
            >
              ❌ 未分配原因
            </button>
            <button
              type="button"
              [class.active-tab]="simulationDetailTab === 'paused'"
              (click)="simulationDetailTab = 'paused'"
            >
              ⏸️ 暂停影响
            </button>
            <button
              type="button"
              [class.active-tab]="simulationDetailTab === 'special'"
              (click)="simulationDetailTab = 'special'"
            >
              🍽️ 特殊餐食
            </button>
            <button
              type="button"
              [class.active-tab]="simulationDetailTab === 'route'"
              (click)="simulationDetailTab = 'route'"
            >
              🗺️ 路线顺序
            </button>
          </div>
          ...
          <div class="sim-date-selector">
            ...
            <select
              [(ngModel)]="simulationViewDate"
              (ngModelChange)="setSimulationViewDate(simulationViewDate)"
            >
          ...
          <div class="modal-body">
            <div *ngIf="simulationDetailTab === 'overview'" class="sim-overview">
              ...
              <div class="sim-section">
                ...
                <div class="sim-tag-list">
                  <div
                    class="sim-tag-item"
                    *ngFor="let tag of getCurrentSimulationStats()?.tagBreakdown"
                  >
                    <span
                      class="tag-chip"
                      [style.background]="tag.color + '20'"
            ...
            <div *ngIf="simulationDetailTab === 'load'" class="sim-load">
              ...
              <div class="sim-load-list">
                <div
                  class="sim-load-item"
                  *ngFor="let v of getCurrentSimulationStats()?.volunteerLoad"
                >
                  <div class="sim-load-header">
                    <strong>{{ v.volunteerName }}</strong>
                    <span [class.overloaded]="v.assigned > v.capacity"
                      >{{ v.assigned }}/{{ v.capacity }}单</span
                    >
                  </div>
                  <div class="load-bar-bg">
                    <div
            ...
            <div *ngIf="simulationDetailTab === 'unassigned'" class="sim-unassigned">
              ...
              <div class="sim-unassigned-list">
                <div
                  class="sim-unassigned-item"
                  *ngFor="let item of getCurrentSimulationStats()?.unassignedReasons"
                >
                ...
                <p
                  class="muted center"
                  *ngIf="!getCurrentSimulationStats()?.unassignedReasons?.length"
                >
                  当日无未分配任务
                </p>
              </div>
            ...
            <div *ngIf="simulationDetailTab === 'paused'" class="sim-paused">
              ...
              <div class="sim-paused-list">
                <div
                  class="sim-paused-item"
                  *ngFor="let elder of getCurrentSimulationStats()?.pausedElders"
                >
                  <div>
              ...
              <p class="muted center" *ngIf="!getCurrentSimulationStats()?.pausedElders?.length">
                当日无暂停送餐的老人
              </p>
            </div>
            ...
            <div *ngIf="simulationDetailTab === 'special'" class="sim-special">
              ...
              <p class="muted">
                当日共有
                <strong>{{ getCurrentSimulationStats()?.specialMealCount || 0 }}</strong> 份特殊餐食
              </p>
              <div class="sim-special-list">
                <div
                  class="sim-special-item"
                  *ngFor="let task of getSimulationTasksForDate(simulationViewDate)"
                >
                  <ng-container *ngIf="task.specialMealNote || elderSpecialNote(task.elderId)">
                    ...
                    <div class="sim-special-note">
                      {{ task.specialMealNote || elderSpecialNote(task.elderId) }}
                    </div>
                  </ng-container>
                </div>
              </div>
            ...
            <div *ngIf="simulationDetailTab === 'route'" class="sim-route">
              ...
              <div class="sim-route-grid">
                <div
                  class="sim-route-column"
                  *ngFor="let group of getCurrentSimulationStats()?.routeOrder"
                >
          ...
          <div class="sim-modal-footer">
            ...
            <button
              type="button"
              class="auto-assign-btn"
              (click)="autoAssignSimulationTasks(); closeSimulationPanel()"
            >
              重新自动分配
            </button>
            <button type="button" class="submit-btn" (click)="submitSimulation()">
              📝 预览差异并提交
            </button>
          </div>
        </div>
      ...
      <div
        class="modal-overlay"
        *ngIf="simulationDiffVisible"
        (click)="closeSimulationDiffPreview()"
      >
        <div class="modal-panel simulation-diff-modal" (click)="$event.stopPropagation()">
          <div class="modal-header">
            <div>
              ...
              <p class="muted">
                {{ simulationData?.startDate }} 至 {{ simulationData?.endDate }} · 共
                {{ getSimulationDatesCount() }} 天 ·
                {{ getSummaryDatesWithChangesCount() }} 天有变化
              </p>
            </div>
            <button type="button" class="ghost sm" (click)="closeSimulationDiffPreview()">
              关闭
            </button>
          </div>
          ...
          <div class="modal-tabs diff-modal-tabs">
            <button
              type="button"
              [class.active-tab]="simulationDiffTab === 'summary'"
              (click)="simulationDiffTab = 'summary'"
            >
              📊 总览
            </button>
            <button
              type="button"
              [class.active-tab]="simulationDiffTab === 'tasks'"
              (click)="simulationDiffTab = 'tasks'"
            >
              ➕➖ 任务增减
              <span
                class="tab-badge"
                *ngIf="getCurrentDiffDayAddedCount() + getCurrentDiffDayRemovedCount() > 0"
              >
            ...
            <button
              type="button"
              [class.active-tab]="simulationDiffTab === 'volunteer'"
              (click)="simulationDiffTab = 'volunteer'"
            >
              👥 分配变更
            ...
            <button
              type="button"
              [class.active-tab]="simulationDiffTab === 'route'"
              (click)="simulationDiffTab = 'route'"
            >
              🗺️ 路线变化
            ...
            <button
              type="button"
              [class.active-tab]="simulationDiffTab === 'paused'"
              (click)="simulationDiffTab = 'paused'"
            >
              ⏸️ 暂停影响
              <span
                class="tab-badge"
                *ngIf="getSummaryTotalPausedNew() + getSummaryTotalPausedResumed() > 0"
              >
            ...
            <button
              type="button"
              [class.active-tab]="simulationDiffTab === 'special'"
              (click)="simulationDiffTab = 'special'"
            >
              🍽️ 特殊餐食
          ...
          <div class="modal-body diff-modal-body">
              ...
              <div class="diff-overview-grid" *ngIf="getCurrentDiffDay()">
                <div class="diff-stat-card">
                  ...
                  <div class="diff-stat-info">
                    ...
                    <div
                      class="diff-stat-row"
                      [class.positive]="getCurrentDiffDayTotalDiff() > 0"
                      [class.negative]="getCurrentDiffDayTotalDiff() < 0"
                    >
                      <span>变化</span>
                      <strong
                        >{{ getCurrentDiffDayTotalDiff() > 0 ? '+' : ''
                        }}{{ getCurrentDiffDayTotalDiff() }}</strong
                      >
                    </div>
                    <small>总任务数</small>
                  </div>
                ...
                <div class="diff-stat-card assigned">
                  ...
                  <div class="diff-stat-info">
                    ...
                    <div
                      class="diff-stat-row"
                      [class.positive]="getCurrentDiffDayAssignedDiff() > 0"
                      [class.negative]="getCurrentDiffDayAssignedDiff() < 0"
                    >
                      <span>变化</span>
                      <strong
                        >{{ getCurrentDiffDayAssignedDiff() > 0 ? '+' : ''
                        }}{{ getCurrentDiffDayAssignedDiff() }}</strong
                      >
                    </div>
                    <small>已分配任务</small>
                  </div>
              ...
              <div class="diff-section" *ngIf="simulationDiffResult">
                ...
                <div class="diff-day-table">
                  ...
                  <div
                    class="diff-day-row"
                    *ngFor="let date of simulationData?.dates"
                    [class.no-changes]="!getDiffDayHasChanges(date)"
                  >
                    <span class="date-cell">{{ date }}</span>
                    <span
                      >{{ getDiffDayTotalBefore(date) }} → {{ getDiffDayTotalAfter(date) }}</span
                    >
                    ...
                    <span class="pause-cell">
                      <ng-container
                        *ngIf="getDiffDayNewPausedCount(date) + getDiffDayResumedCount(date) > 0"
                      >
                        <span
                          class="pause-mini-tag pause-new-mini"
                          *ngIf="getDiffDayNewPausedCount(date) > 0"
                          >新停{{ getDiffDayNewPausedCount(date) }}</span
                        >
                        <span
                          class="pause-mini-tag pause-resume-mini"
                          *ngIf="getDiffDayResumedCount(date) > 0"
                          >恢复{{ getDiffDayResumedCount(date) }}</span
                        >
                      </ng-container>
                      <span
                        class="muted sm"
                    ...
                    <span>
                      <span
                        class="status-tag"
                        *ngIf="getDiffDayHasChanges(date)"
            ...
            <div *ngIf="simulationDiffTab === 'tasks'" class="diff-tasks">
                  ...
                  <div class="diff-item added" *ngFor="let item of getCurrentDiffDayAddedTasks()">
                    <div class="diff-item-main">
                      <div class="diff-item-title">
                        <strong (click)="locateElderInDiff(item.elderId)" class="clickable">{{
                          item.elderName
                        }}</strong>
                        <span class="route-order-tag" *ngIf="item.routeOrder"
                          >#{{ item.routeOrder }}</span
                        >
                        <span
                          class="diff-volunteer"
                          *ngIf="item.volunteerName"
                          (click)="locateVolunteerInDiff(item.volunteerId!)"
                          class="clickable"
                          >配送：{{ item.volunteerName }}</span
                        >
                        <span class="diff-volunteer unassigned" *ngIf="!item.volunteerName"
                          >未分配</span
                        >
                        <span
                          class="reason-tag resumed"
                          *ngIf="item.addReason === 'resumed-from-pause'"
                          >⏪ 恢复送餐</span
                        >
                        <span class="reason-tag new" *ngIf="item.addReason === 'new-elder'"
                          >👤 新老人</span
                        >
                        <span class="reason-tag new" *ngIf="item.addReason === 'delivery-day-added'"
                          >📅 新增配送日</span
                        >
                      </div>
                      <small>{{ item.elderAddress }}</small>
                      <div class="special-note" *ngIf="item.specialMealNote">
              ...
              <div class="diff-section" *ngIf="getCurrentDiffDayRemovedCount() > 0">
                ...
                <div class="diff-list">
                  <div
                    ...
                    *ngFor="let item of getCurrentDiffDayRemovedTasks()"
                  >
                    <div class="diff-item-main">
                      <div class="diff-item-title">
                        <strong (click)="locateElderInDiff(item.elderId)" class="clickable">{{
                        ...
                        <span class="route-order-tag" *ngIf="item.routeOrder"
                          >#{{ item.routeOrder }}</span
                        >
                        <span class="diff-volunteer" *ngIf="item.volunteerName"
                          >原配送：{{ item.volunteerName }}</span
                        >
                        <span class="diff-volunteer unassigned" *ngIf="!item.volunteerName"
                          >原未分配</span
                        >
                        <span class="reason-tag paused" *ngIf="item.removeReason === 'paused'"
                          >⏸️ 转为暂停</span
                        >
                        <span
                          class="reason-tag removed"
                          *ngIf="item.removeReason === 'elder-removed'"
                          >👤 移除老人</span
                        >
                        <span
                          class="reason-tag removed"
                          *ngIf="item.removeReason === 'delivery-day-removed'"
                          >📅 移除配送日</span
                        >
                      </div>
                      <small>{{ item.elderAddress }}</small>
              ...
              <p
                class="muted center"
                *ngIf="getCurrentDiffDayAddedCount() === 0 && getCurrentDiffDayRemovedCount() === 0"
              >
                当日无任务增减
              </p>
            </div>
            ...
            <div *ngIf="simulationDiffTab === 'volunteer'" class="diff-volunteer-change">
              <div class="diff-section" *ngIf="getCurrentDiffDayVolunteerCount() > 0">
                ...
                <div class="diff-list">
                  <div
                    class="diff-item volunteer-change"
                    *ngFor="let item of getCurrentDiffDayVolunteerChanges()"
                  >
                    <div class="diff-item-main">
                      <div class="diff-item-title">
                        <strong (click)="locateElderInDiff(item.elderId)" class="clickable">{{
                      ...
                      <div class="change-arrow-row">
                        <div class="change-box old">
                          <small>原分配</small>
                          <strong
                            *ngIf="item.oldVolunteerName"
                            (click)="locateVolunteerInDiff(item.oldVolunteerId!)"
                            class="clickable"
                            >{{ item.oldVolunteerName }}</strong
                          >
                        ...
                        <div class="change-box new">
                          <small>新分配</small>
                          <strong
                            *ngIf="item.newVolunteerName"
                            (click)="locateVolunteerInDiff(item.newVolunteerId!)"
                            class="clickable"
                            >{{ item.newVolunteerName }}</strong
                          >
              ...
              <p class="muted center" *ngIf="getCurrentDiffDayVolunteerCount() === 0">
                当日无志愿者分配变更
              </p>
            </div>
            ...
            <div *ngIf="simulationDiffTab === 'route'" class="diff-route-change">
              <div class="diff-section" *ngIf="getCurrentDiffDayRouteCount() > 0">
                ...
                <div class="route-compare-grid">
                  <div
                    ...
                    *ngFor="let rc of getCurrentDiffDayRouteChanges()"
                  >
                    <div class="route-compare-header">
                      <strong (click)="locateVolunteerInDiff(rc.volunteerId)" class="clickable">{{
                        rc.volunteerName
                      }}</strong>
                    ...
                    <div class="route-compare-body">
                      <div class="route-col old">
                        ...
                        <div class="route-compare-list">
                          <div
                            class="route-compare-item"
                            *ngFor="let task of rc.oldOrder"
                            [class.moved]="hasMovedTask(rc.movedTasks, task.elderId)"
                          >
                            <span class="route-num">{{ task.position }}</span>
                            <span (click)="locateElderInDiff(task.elderId)" class="clickable">{{
                              task.elderName
                            }}</span>
                          </div>
                      ...
                      <div class="route-col new">
                        ...
                        <div class="route-compare-list">
                          ...
                          >
                            ...
                            <span
                              class="moved-tag"
                              *ngIf="hasMovedTask(rc.movedTasks, task.elderId)"
                            >
                              {{ getMovedTaskByElder(rc.movedTasks, task.elderId)?.oldPos }}→{{
                                getMovedTaskByElder(rc.movedTasks, task.elderId)?.newPos
                              }}
                            </span>
                          </div>
                    ...
                    <div class="route-moved-list" *ngIf="rc.movedTasks.length > 0">
                      ...
                      <div class="moved-item" *ngFor="let m of rc.movedTasks">
                        <span (click)="locateElderInDiff(m.elderId)" class="clickable">{{
                          m.elderName
                        }}</span>
              ...
              <p class="muted center" *ngIf="getCurrentDiffDayRouteCount() === 0">
                当日无路线顺序变化
              </p>
            </div>
            ...
            <div *ngIf="simulationDiffTab === 'paused'" class="diff-paused">
              <div class="diff-section">
                ...
                <div class="diff-list" *ngIf="getCurrentDiffDayPausedCount() > 0">
                  <div
                    class="diff-item paused-item"
                    *ngFor="let item of getCurrentDiffDayPausedChanges()"
                    [class.new-pause]="item.changeType === 'pause-new'"
                    [class.resume]="item.changeType === 'pause-resume'"
                  >
                    <div class="diff-item-main">
                      <div class="diff-item-title">
                        <strong (click)="locateElderInDiff(item.elderId)" class="clickable">{{
                        ...
                        <span class="pause-tag new" *ngIf="item.changeType === 'pause-new'"
                          >新增暂停</span
                        >
                        <span class="pause-tag resume" *ngIf="item.changeType === 'pause-resume'"
                          >恢复送餐</span
                        >
                        <span
                          class="pause-tag keep"
                          *ngIf="item.changeType === 'pause-unchanged' && item.isPaused"
                          >持续暂停</span
                        >
                        <span class="source-tag" *ngIf="item.changeSource">{{
                          getPauseChangeSourceText(item.changeSource)
                        }}</span>
                      </div>
                      <small>{{ item.address }}</small>
                      <small class="contact">📞 {{ item.contact }}</small>
                      <div class="pause-status-row">
                        <span class="pause-status" [class.active]="item.wasPaused"
                          >提交前：{{ item.wasPaused ? '已暂停' : '正常送餐' }}</span
                        >
                        <span class="pause-arrow">→</span>
                        <span class="pause-status" [class.active]="item.isPaused"
                          >提交后：{{ item.isPaused ? '暂停送餐' : '正常送餐' }}</span
                        >
                      </div>
                ...
                <p class="muted center" *ngIf="getCurrentDiffDayPausedCount() === 0">
                  当日无暂停送餐的老人
                </p>
              </div>
            </div>
            ...
            <div *ngIf="simulationDiffTab === 'special'" class="diff-special">
              <div class="diff-section" *ngIf="getCurrentDiffDaySpecialCount() > 0">
                ...
                <div class="diff-list">
                  <div
                    class="diff-item special-change"
                    *ngFor="let item of getCurrentDiffDaySpecialChanges()"
                  >
                    <div class="diff-item-main">
                      <div class="diff-item-title">
                        <strong (click)="locateElderInDiff(item.elderId)" class="clickable">{{
                        ...
                        <span class="special-tag new" *ngIf="item.changeType === 'special-new'"
                          >新增特殊餐</span
                        >
                        <span
                          class="special-tag removed"
                          *ngIf="item.changeType === 'special-removed'"
                          >取消特殊餐</span
                        >
                        <span
                          class="special-tag changed"
                          *ngIf="item.changeType === 'special-changed'"
                          >餐食调整</span
                        >
                        <span class="source-tag" *ngIf="item.changeSource">{{
                          getSpecialChangeSourceText(item.changeSource)
                        }}</span>
                      </div>
                      <div class="source-compare-row" *ngIf="item.oldSource || item.newSource">
                        <span class="source-label" *ngIf="item.oldSource"
                          >原来源：{{ getSpecialChangeSourceText(item.oldSource) }}</span
                        >
                        <span
                          class="source-arrow"
                          *ngIf="
                            item.oldSource && item.newSource && item.oldSource !== item.newSource
                          "
                          >→</span
                        >
                        <span
                          class="source-label"
                          *ngIf="item.newSource && item.oldSource !== item.newSource"
                          >新来源：{{ getSpecialChangeSourceText(item.newSource) }}</span
                        >
                      </div>
                      ...
                      <div class="change-detail-row" *ngIf="item.relatedTempChangeReason">
                        ...
                        <span class="detail-text temp-reason">{{
                          item.relatedTempChangeReason
                        }}</span>
                      </div>
                    </div>
                  </div>
              ...
              <p class="muted center" *ngIf="getCurrentDiffDaySpecialCount() === 0">
                当日无特殊餐食变更
              </p>
            </div>
          </div>
          ...
          <div class="diff-modal-footer">
            <button type="button" class="ghost" (click)="closeSimulationDiffPreview()">
              返回修改
            </button>
            <button
              type="button"
              class="submit-btn confirm-btn"
              (click)="confirmSubmitSimulation()"
            >
      ...
      <div
        class="sync-toast"
        *ngIf="syncToastVisible"
        [class.toast-warn]="lastSyncType === 'warn'"
        [class.toast-error]="lastSyncType === 'error'"
        [class.toast-success]="lastSyncType === 'success'"
      >
      ...
      <div
        ...
        *ngIf="showDraftConflictPanel"
        (click)="showDraftConflictPanel = false"
      >
        <div class="conflict-modal" (click)="$event.stopPropagation()">
          <header class="conflict-modal-header">
            ...
            <button type="button" class="close-btn" (click)="showDraftConflictPanel = false">
              ✕
            </button>
          </header>
          ...
          <div class="conflict-body">
            <div class="conflict-content" style="width: 100%;">
              <div class="conflict-type-header">
                ...
                <div class="global-resolution-bar" style="margin-left: auto;">
                  <button
                    type="button"
                    class="ghost"
                    (click)="resolveAllDraftConflicts('keep-local')"
                  >
                    📌 全部保留本地
                  </button>
                  <button
                    type="button"
                    class="ghost"
                    (click)="resolveAllDraftConflicts('adopt-remote')"
                  >
                    🔄 全部采用远程
                  </button>
                </div>
              ...
              <div class="conflict-record-list" *ngFor="let conflict of offlineDraftConflicts">
                <div
                  class="conflict-draft-card"
                  style="border: 1px solid #e2e7da; border-radius: 8px; padding: 16px; margin-bottom: 12px; background: #fff;"
                >
                  <div
                    class="conflict-draft-header"
                    style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;"
                  >
                    <div>
                      <strong style="font-size: 15px;"
                        >任务 #{{ conflict.taskId.slice(-4) }}</strong
                      >
                      <span
                        class="conflict-type-tag"
                        style="margin-left: 8px; padding: 2px 8px; border-radius: 4px; font-size: 12px; background: #c7545420; color: #c75454;"
                      >
                  ...
                  <div
                    class="conflict-draft-detail"
                    style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 12px;"
                  >
                    <div
                      class="local-version"
                      style="background: #f0f8f0; border: 1px solid #c4e0c4; border-radius: 8px; padding: 12px;"
                    >
                      <h4 style="margin: 0 0 8px; font-size: 13px; color: #4a9f6d;">
                        📝 本地离线修改
                      </h4>
                      <p style="margin: 4px 0; font-size: 13px;">
                        <strong>状态：</strong>{{ conflict.deliveryStatus }}
                      </p>
                      <p style="margin: 4px 0; font-size: 13px;">
                        <strong>备注：</strong>{{ conflict.exceptionNote || '无' }}
                      </p>
                      <p style="margin: 4px 0; font-size: 12px; color: #65715f;">
                        修改时间：{{ conflict.createdAt }}
                      </p>
                    </div>
                    <div
                      class="remote-version"
                      style="background: #fff5f5; border: 1px solid #f0c4c4; border-radius: 8px; padding: 12px;"
                    >
                      <h4 style="margin: 0 0 8px; font-size: 13px; color: #c75454;">
                        ⚠️ 远程当前状态
                      </h4>
                      <p style="margin: 4px 0; font-size: 13px;">
                        <strong>状态：</strong>{{ conflict.conflictInfo?.remoteStatus || '未知' }}
                      </p>
                      <p style="margin: 4px 0; font-size: 13px; color: #c75454;">
                        <strong>警告：</strong>任务已完成，本地修改可能覆盖已送达状态
                      </p>
                      <p style="margin: 4px 0; font-size: 12px; color: #65715f;">
                        更新时间：{{ conflict.conflictInfo?.remoteUpdatedAt || '未知' }}
                      </p>
                    </div>
                  </div>

                  <div
                    class="conflict-draft-actions"
                    style="display: flex; gap: 8px; justify-content: flex-end;"
                  >
                    <button
                      type="button"
                      class="ghost"
                      (click)="resolveDraftConflict(conflict.id, 'adopt-remote')"
                    >
                      🔄 采用远程版本
                    </button>
                    <button
                      ...
                      class="primary"
                      style="background: #5a8fd9;"
                      (click)="resolveDraftConflict(conflict.id, 'keep-local')"
                    >
                      📌 保留本地修改
                    </button>
                  </div>
      ...
      <div
        class="conflict-modal-backdrop"
        *ngIf="conflictPanelVisible"
        (click)="dismissConflictPanel()"
      >
        <div class="conflict-modal" (click)="$event.stopPropagation()">
          <header class="conflict-modal-header">
            <div>
          ...
          <div class="conflict-body">
            <aside class="conflict-groups">
              ...
              <div class="global-resolution-bar">
                <button type="button" class="ghost" (click)="keepAllLocal()">
                  📌 全部保留本地
                </button>
                <button type="button" class="ghost" (click)="adoptAllRemote()">
                  🔄 全部采用新数据
                </button>
              </div>
            </aside>
              ...
              <div class="conflict-type-header">
                ...
                <div class="resolution-switch">
                  <label>
                    <input
                      type="radio"
                      [name]="'grp-' + selectedConflictGroupIndex"
                  ...
                  <label>
                    <input
                      ...
                      (change)="
                        setGroupDefaultResolution(selectedConflictGroupIndex, 'adopt-remote')
                      "
                    />
                    <span>采用新数据</span>
                  </label>
                  <label>
                    <input
                      ...
                      (change)="
                        setGroupDefaultResolution(selectedConflictGroupIndex, 'field-level')
                      "
                    />
                    <span>按字段选择</span>
                  </label>
                  ...
                  >
                    <div class="rc-row">
                      ...
                      <span class="rc-resolution rc-{{ rc.resolution }}">
                        {{
                          rc.resolution === 'keep-local'
                            ? '保留本地'
                            : rc.resolution === 'adopt-remote'
                              ? '采用新数据'
                              : '按字段'
                        }}
                      </span>
            ...
            <section class="conflict-field-detail" *ngIf="selectedRecordConflict">
              <div class="selected-record-header">
                ...
                <div class="record-resolution-switch">
                  <label>
                    <input
                      type="radio"
                      [name]="'rec-' + selectedRecordConflict.recordId"
                  ...
                  <label>
                    <input
                      ...
                      (change)="
                        setRecordResolution(selectedRecordConflict.recordId, 'adopt-remote')
                      "
                    />
                    <span>采用新数据</span>
                  </label>
              ...
              <div class="field-diff-table">
                <div class="diff-row diff-header">
                  ...
                  <span
                    class="diff-choice"
                    *ngIf="selectedRecordConflict.resolution === 'field-level'"
                    >选择</span
                  >
                </div>
                <div class="diff-row" *ngFor="let fc of selectedRecordConflict.fieldConflicts">
                  <span class="diff-field"
                    ><strong>{{ fc.field }}</strong></span
                  >
                  <span
                    class="diff-local"
                    [class.chosen]="
                      selectedRecordConflict.resolution === 'keep-local' ||
                      (selectedRecordConflict.resolution === 'field-level' &&
                        selectedRecordConflict.fieldResolutions?.[fc.field] === 'local')
                    "
                  >
                  ...
                  <span
                    class="diff-remote"
                    [class.chosen]="
                      selectedRecordConflict.resolution === 'adopt-remote' ||
                      (selectedRecordConflict.resolution === 'field-level' &&
                        selectedRecordConflict.fieldResolutions?.[fc.field] === 'remote')
                    "
                  >
                  ...
                  >
                    <label>
                      <input
                        ...
                        (change)="
                          setFieldChoice(selectedRecordConflict.recordId, fc.field, 'local')
                        "
                      />
                      <span>本地</span>
                    </label>
                    <label>
                      <input
                        ...
                        [checked]="selectedRecordConflict.fieldResolutions?.[fc.field] === 'remote'"
                        (change)="
                          setFieldChoice(selectedRecordConflict.recordId, fc.field, 'remote')
                        "
                      />
                      <span>新数据</span>
                    </label>
          ...
          <footer class="conflict-modal-footer">
            ...
            <button type="button" class="confirm-btn" (click)="applyAllConflicts()">
              ✅ 应用所有选择并合并
            </button>
          </footer>
        </div>
    ...
    `
      * {
      ...
      .hero {
        ...
        padding: 30px;
        border-radius: 8px;
        background: linear-gradient(135deg, #2c5144, #83714b);
        color: white;
      }
      .hero p {
        margin: 0 0 6px;
      ...
      .panel {
        ...
        border-radius: 8px;
        padding: 18px;
        box-shadow: 0 10px 28px rgb(38 49 34 / 0.07);
      }
      form.panel {
        display: flex;
      ...
      .taskList article {
        display: grid;
        grid-template-columns: 1.2fr 240px auto;
        gap: 12px;
        align-items: center;
      ...
      .modal-panel {
        ...
        flex-direction: column;
        overflow: hidden;
        box-shadow: 0 30px 80px rgba(36, 41, 35, 0.25);
      }
      .modal-header {
        display: flex;
        justify-content: space-between;
      ...
      .elder-edit-card {
        ...
        border-color: #315448 !important;
        background: #f4f7ee !important;
      }
      .elder-edit-card input {
        width: 100%;
      }
      .elder-edit-actions {
        display: flex;
      ...
      .exc-status-item strong {
        display: block;
        font-size: 22px;
        margin-bottom: 2px;
      }
      .exc-status-item span {
        font-size: 12px;
        color: #65715f;
      }
      .exc-list {
      ...
      .exc-filter-bar select,
      .exc-filter-bar input {
        flex: 1;
        min-width: 120px;
        padding: 8px 10px;
        font-size: 13px;
      ...
      .preview-stats .stat.new {
        background: #e8f3ec;
        color: #4a9f6d;
      }
      .preview-stats .stat.overwrite {
        background: #fdf3e0;
        color: #b3832a;
      }
      .preview-stats .stat.duplicate {
        background: #eff1ec;
        color: #8a9783;
      }
      ...
      .phone-status-item strong {
        display: block;
        font-size: 20px;
        margin-bottom: 2px;
        color: #315448;
      }
      .phone-status-item span {
        font-size: 12px;
        color: #65715f;
      }
      ...
      .callback-item-result label {
        font-weight: 600;
        color: #3d4a38;
        white-space: nowrap;
      }
      .callback-item-result span {
        color: #4a5a45;
        line-height: 1.5;
      }
      ...
      .conflict-groups li:hover {
        background: #e8ede1;
      }
      .conflict-groups li.active {
        background: #2c5144;
        color: white;
      }
      .badge {
      ...
      .conflict-groups li.active .badge {
        background: #ff6b6b;
      }
      ...
      .resolution-switch,
      .record-resolution-switch {
        display: flex;
        flex-wrap: wrap;
        gap: 12px;
      }
      .resolution-switch label,
      .record-resolution-switch label,
      .diff-choice label {
        display: inline-flex;
        align-items: center;
        gap: 6px;
      ...
      .resolution-switch label:hover,
      .record-resolution-switch label:hover {
        background: #eef3e7;
      }

      .conflict-record-list ul {
        list-style: none;
        margin: 0;
        padding: 0;
        display: flex;
      ...
      .conflict-record-list li:hover {
        border-color: #2c5144;
        background: #f4f7f0;
      }
      .conflict-record-list li.active {
        border-color: #2c5144;
        background: #eaf2e4;
        box-shadow: 0 0 0 2px rgba(44, 81, 68, 0.15);
      }
      .rc-row {
        display: flex;
        justify-content: space-between;
      ...
      .rc-resolution.rc-keep-local {
        background: #d9f0e0;
        color: #2d7a4c;
      }
      .rc-resolution.rc-adopt-remote {
        background: #d9e4f5;
        color: #2d5b9a;
      }
      .rc-resolution.rc-field-level {
        background: #f5ecd9;
        color: #8a5a1d;
      }
      .conflict-record-list li small {
        color: #65715f;
        font-size: 12px;
      }
      ...
      .sim-summary-item.ok strong {
        color: #4a9f6d;
      }
      .sim-summary-item.warn strong {
        color: #d9a84a;
      }
      .sim-summary-item.muted strong {
        color: #8a9783;
      }
      .sim-summary-item.special strong {
        color: #b36a2e;
      }
      ...
      .sim-stat-card.ok .sim-stat-info strong {
        color: #4a9f6d;
      }
      .sim-stat-card.warn .sim-stat-info strong {
        color: #d9a84a;
      }
      .sim-stat-card.muted .sim-stat-info strong {
        color: #8a9783;
      }
      .sim-stat-card.special .sim-stat-info strong {
        color: #b36a2e;
      }
      ...
      .sim-load-header strong {
        font-size: 14px;
      }
      .sim-load-header span {
        font-size: 13px;
        color: #65715f;
      }
      .sim-load-header span.overloaded {
        color: #c75454;
        font-weight: 600;
      }
      ...
      .sim-unassigned-elder strong {
        font-size: 14px;
        display: block;
      }
      .sim-unassigned-elder small {
        color: #65715f;
      }
      .sim-unassigned-reason {
        font-size: 13px;
        color: #c75454;
        text-align: right;
        flex-shrink: 0;
      ...
      .sim-paused-item strong {
        font-size: 14px;
        display: block;
      }
      .sim-paused-item small {
        color: #65715f;
      }
      .sim-paused-contact {
        font-size: 13px;
      ...
      .sim-special-item strong {
        font-size: 14px;
        display: block;
      }
      .sim-special-item small {
        color: #65715f;
      }
      .sim-special-note {
        font-size: 13px;
      ...
      .sim-route-header strong {
        font-size: 14px;
      }
      .sim-route-header span {
        font-size: 12px;
        color: #65715f;
      }
      .sim-route-list {
      ...
      .sim-route-info strong {
        font-size: 13px;
        display: block;
      }
      .sim-route-info small {
        font-size: 11px;
        color: #65715f;
      }
      ...
      .diff-sum-item.added strong {
        color: #4a9f6d;
      }
      .diff-sum-item.removed strong {
        color: #c75454;
      }
      .diff-sum-item.volunteer strong {
        color: #5a8fd9;
      }
      .diff-sum-item.route strong {
        color: #9a6bd9;
      }
      .diff-sum-item.paused strong {
        color: #d9a84a;
      }
      .diff-sum-item.special strong {
        color: #b36a2e;
      }
      ...
      .diff-day-tab.has-changes:not(.active) {
        border-color: #ffb74d;
        background: #fff8ed;
      }
      .diff-day-badge {
      ...
      .diff-day-tab.active .diff-day-badge {
        background: rgba(255, 255, 255, 0.25);
        color: #fff;
      }
      ...
      .diff-stat-row strong {
        font-size: 15px;
      }
      .diff-stat-row.positive strong {
        color: #4a9f6d;
      }
      .diff-stat-row.negative strong {
        color: #c75454;
      }
      .diff-stat-info small {
        font-size: 12px;
        color: #65715f;
        display: block;
        margin-top: 4px;
      }
      .diff-stat-card.assigned {
        background: #edf7f0;
      }
      ...
      .diff-stat-card.volunteer {
        background: #eef3fb;
        border-color: #b9cfee;
      }
      .diff-stat-card.route {
        background: #f3eff8;
        border-color: #cebae9;
      }
      ...
      .diff-day-row {
        display: grid;
        grid-template-columns: 1.1fr 1.2fr 0.6fr 0.6fr 0.85fr 0.85fr 1fr 0.8fr 0.75fr;
        gap: 0;
      }
      .diff-day-row.header {
        background: #f0f2ed;
      ...
      .diff-day-row:not(.header):last-child span {
        border-bottom: none;
      }
      .diff-day-row:not(.header):hover {
        background: #fafbf7;
      }
      .diff-day-row.no-changes {
        opacity: 0.6;
      }
      .date-cell {
        font-weight: 600;
      ...
      .diff-item.added {
        border-left: 4px solid #4a9f6d;
        background: #f5faf6;
      }
      .diff-item.removed {
        border-left: 4px solid #c75454;
        background: #fbf4f4;
      }
      .diff-item.volunteer-change {
        border-left: 4px solid #5a8fd9;
        background: #f4f8fc;
      }
      .diff-item.special-change {
        border-left: 4px solid #b36a2e;
        background: #fcf6ee;
      }
      .diff-item.paused-item {
        border-left: 4px solid #d9a84a;
        background: #fcf7ed;
      }
      .diff-item.paused-item.new-pause {
        border-left-color: #c75454;
        background: #fbf4f4;
      }
      .diff-item.paused-item.resume {
        border-left-color: #4a9f6d;
        background: #f5faf6;
      }
      .diff-item-main {
        flex: 1;
      ...
      .change-detail-row .detail-label {
        color: #8a9783;
        flex-shrink: 0;
      }
      .change-detail-row .detail-text {
        color: #3d4a38;
      }
      .change-detail-row .detail-text.temp-reason {
        color: #9a6bd9;
        font-style: italic;
      }
      .source-compare-row {
      ...
      .source-compare-row .source-label {
        font-size: 11px;
        padding: 2px 8px;
        border-radius: 10px;
        background: #f0f2ed;
      ...
      .source-compare-row .source-arrow {
        color: #8a9783;
        font-size: 12px;
      }
      ...
      .route-col.new .route-num {
        background: #4a9f6d;
        color: #fff;
      }
      .route-col.old .route-num {
        background: #c75454;
        color: #fff;
      }
      .moved-tag {
      ...
      .moved-item:last-child {
        border-bottom: none;
      }
      .moved-item span:last-child {
        color: #b36a2e;
        font-weight: 500;
      }
      ...
      .diff-modal-footer .confirm-btn {
        background: linear-gradient(135deg, #4a9f6d, #3a8f5d);
        padding: 10px 24px;
        font-size: 14px;
      }
      .diff-modal-footer .confirm-btn:hover {
        background: linear-gradient(135deg, #3a8f5d, #2e7f4d);
      }

      .toast-success {
        background: #4a9f6d !important;
        color: #fff;
      }
      ...
      @media (max-width: 900px) {
        ...
        .diff-day-row {
          grid-template-columns: 1.5fr 1fr repeat(4, 0.8fr);
          font-size: 11px;
        }
        .diff-day-row span {
          padding: 7px 6px;
      ...
      .temp-change-existing {
        margin-bottom: 16px;
      }
      .temp-change-existing h3 {
        font-size: 14px;
        margin: 0 0 8px;
        color: #65715f;
      }
      .temp-change-list-item {
        display: flex;
        justify-content: space-between;
        align-items: flex-start;
        padding: 10px;
      ...
      .temp-change-list-info {
        display: flex;
        flex-direction: column;
        gap: 2px;
      }
      .temp-change-list-info strong {
        font-size: 13px;
      }
      .temp-change-list-summary {
        font-size: 12px;
        color: #b36a2e;
      }
      .temp-change-list-actions {
        display: flex;
        gap: 4px;
      }
      .temp-change-form h3 {
      ...
      .temp-change-conflict {
        margin: 12px 0;
        padding: 12px;
        border-radius: 8px;
        background: #fff7ef;
        border: 1px solid #d78b63;
      }
      ...
      .temp-change-card {
        border-left: 3px solid #d9a84a !important;
      ...
      .temp-change-manage-modal {
        max-width: 900px;
        width: 95vw;
      }
      .temp-change-manage-modal .modal-body {
        max-height: 75vh;
        overflow-y: auto;
        padding: 0;
      }
      ...
      .tc-stat-num.today-num {
        color: #c75454;
      }
      .tc-stat-num.upcoming-num {
        color: #d9a84a;
      }
      .tc-stat-num.expired-num {
        color: #8a9481;
      }
      .tc-stat-label {
        font-size: 12px;
      ...
      .tc-change-card:last-child {
        border-bottom: none;
      }
      .tc-change-card.expanded {
        background: #fffdf5;
      }
      ...
      @keyframes tc-today-pulse {
        0%,
        100% {
          box-shadow: 0 0 0 0 rgba(199, 84, 84, 0.3);
        }
        50% {
          box-shadow: 0 0 0 4px rgba(199, 84, 84, 0.08);
        }
      }
...
export class App implements AfterViewChecked, OnInit {
  ...
  private handleIncomingConflicts(groups: SyncConflictGroup[]) {
    ...
    this.showSyncToast(
      `检测到 ${groups.reduce((s, g) => s + g.conflicts.length, 0)} 个数据冲突，请处理`,
      'warn',
    );
  }
  ...
  private refreshDataTypeFromStorage(dataType: SyncDataType) {
    switch (dataType) {
      case 'elders':
        this.elders = this.sync.readLocalData<Elder[]>('elders') || this.elders;
        this.sync.captureLocalSnapshot('elders', this.elders);
        break;
      case 'volunteers':
        this.volunteers = this.sync.readLocalData<Volunteer[]>('volunteers') || this.volunteers;
        this.sync.captureLocalSnapshot('volunteers', this.volunteers);
        break;
      case 'tasks':
        this.tasks = this.sync.readLocalData<MealTask[]>('tasks') || this.tasks;
        this.sync.captureLocalSnapshot('tasks', this.tasks);
        break;
      case 'mealTags':
        this.mealTags = this.sync.readLocalData<MealTag[]>('mealTags') || this.mealTags;
        this.sync.captureLocalSnapshot('mealTags', this.mealTags);
        break;
      case 'exceptionRecords':
        this.exceptionRecords =
          this.sync.readLocalData<ExceptionRecord[]>('exceptionRecords') || this.exceptionRecords;
        this.sync.captureLocalSnapshot('exceptionRecords', this.exceptionRecords);
        break;
      case 'visitRecords':
        this.visitRecords =
          this.sync.readLocalData<VisitRecord[]>('visitRecords') || this.visitRecords;
        this.sync.captureLocalSnapshot('visitRecords', this.visitRecords);
        break;
      case 'phoneNotifications':
        this.phoneNotifications =
          this.sync.readLocalData<PhoneNotification[]>('phoneNotifications') ||
          this.phoneNotifications;
        this.sync.captureLocalSnapshot('phoneNotifications', this.phoneNotifications);
        break;
      case 'callbackTasks':
        this.callbackTasks =
          this.sync.readLocalData<CallbackTask[]>('callbackTasks') || this.callbackTasks;
        this.sync.captureLocalSnapshot('callbackTasks', this.callbackTasks);
        break;
      case 'kanbanSort':
        this.kanbanSort = this.sync.readLocalData<KanbanSortMap>('kanbanSort') || this.kanbanSort;
        this.sync.captureLocalSnapshot('kanbanSort', this.kanbanSort);
        break;
      case 'temporaryDeliveryChanges':
        this.temporaryDeliveryChanges =
          this.sync.readLocalData<TemporaryDeliveryChange[]>('temporaryDeliveryChanges') ||
          this.temporaryDeliveryChanges;
        this.sync.captureLocalSnapshot('temporaryDeliveryChanges', this.temporaryDeliveryChanges);
        break;
    }
  ...
  elders: Elder[] = [
    {
      ...
      name: '苏阿姨',
      preference: '少盐软饭',
      mealTags: ['low-salt', 'soft-food'],
      address: '松桂里3栋201',
      contact: '女儿13800001111',
      note: '午餐需敲门等候',
  ...
  simulationDetailTab: 'overview' | 'load' | 'unassigned' | 'paused' | 'special' | 'route' =
    'overview';
  ...
  exceptionForm: Omit<
    ExceptionRecord,
    'id' | 'taskId' | 'elderId' | 'date' | 'createdAt' | 'updatedAt'
  > = {
    category: '无人应答',
    severity: '一般',
    description: '',
  ...
  onPrepTaskUpdated(data: { taskId: string; status: MealTask['status']; exception: string }) {
    this.tasks = this.tasks.map((t) =>
      t.id === data.taskId
        ? { ...t, status: data.status, exception: data.exception || t.exception }
        : t,
    );
    this.save();
  }
  ...
  resolveDraftConflict(draftId: string, resolution: 'keep-local' | 'adopt-remote') {
    ...
    this.showSyncToast(resolution === 'keep-local' ? '已保留本地修改' : '已采用远程版本', 'info');
  }
    ...
    if (result.taskUpdated && result.taskUpdated.id) {
      ...
      this.tasks = this.tasks.map((t) => {
        ...
        return {
          ...
          ...(result.taskUpdated!.exception !== undefined
            ? { exception: result.taskUpdated!.exception }
            : {}),
          ...(result.taskUpdated!.volunteerId !== undefined
            ? { volunteerId: result.taskUpdated!.volunteerId }
            : {}),
          ...(result.taskUpdated!.isManuallyModified !== undefined
            ? { isManuallyModified: result.taskUpdated!.isManuallyModified }
            : {}),
        };
      });
      this.save();
  ...
  private isExceptionDuplicate(
    taskId: string,
    source: ExceptionSource,
    category?: ExceptionCategory,
  ): boolean {
    return this.exceptionRecords.some(
      (r) => r.taskId === taskId && r.source === source && (!category || r.category === category),
    );
  }

  private isNotificationDuplicate(
    taskId: string,
    source: ExceptionSource,
    targetId: string,
  ): boolean {
    return this.phoneNotifications.some(
      (n) => n.taskId === taskId && n.source === source && n.targetId === targetId,
    );
  }

  private buildPhoneNotification(
    task: MealTask,
    elder: Elder,
    remark: string,
    source: ExceptionSource,
  ...
  private isCallbackDuplicate(notificationId: string, status: string): boolean {
    return this.callbackTasks.some(
      (c) => c.notificationId === notificationId && c.status !== '已完成' && c.status !== '已取消',
    );
  }
    ...
    this.volunteers = [
      {
        id: crypto.randomUUID(),
        ...this.volunteerForm,
        capacity: Number(this.volunteerForm.capacity || 1),
      },
      ...this.volunteers,
    ];
    this.volunteerForm = {
  ...
  generateTasks() {
    const existing = new Set(
      this.tasks.filter((task) => task.date === this.taskDate).map((task) => task.elderId),
    );
  ...
  exceptionTasks() {
    const resolvedTaskIds = new Set(
      this.exceptionRecords.filter((r) => r.status === '已解决').map((r) => r.taskId),
    );
  ...
  autoAssignTasks() {
    ...
    for (const task of unassigned) {
      ...
      if (candidates.length === 0) {
        const matchingVolunteers = this.volunteers.filter(
          (v) => v.area.trim() && elder.address.trim() && elder.address.includes(v.area),
        );
        let reason = '';
  ...
  private getEffectiveSpecialMealNote(
    ...
    note: string;
    source?: SimDiffSpecialMealItem['oldSource'];
  ...
  private generateSimulationTasks() {
    ...
    for (const date of this.simulationData.dates) {
      ...
      const created = dayElders
        ...
        .map((elder) => {
          const special = this.getEffectiveSpecialMealNote(
            undefined,
            elder,
            date,
            this.temporaryDeliveryChanges,
  ...
  private autoAssignSimulationForDate(date: string) {
    ...
    for (const task of unassigned) {
      ...
      const elder = this.applyTempChangeToElderFromList(
        rawElder,
        date,
        this.temporaryDeliveryChanges,
      );
    ...
    for (const date of this.simulationData.dates) {
      ...
      for (const elderId of allElderIds) {
        ...
        const effectiveRealElder = baselineElder
          ? this.applyTempChangeToElderFromList(
              baselineElder,
              date,
        ...
        const realRouteOrder =
          realVolId && realTask ? realSort.indexOf(realTask.id) + 1 : undefined;
        ...
        if (!realTask && simTask) {
          ...
          if (wasPaused && !isPausedNow) {
            ...
            const elderHasAnyRealTask = this.simulationData.baselineTasks.some(
              (t) => t.elderId === elderId,
            );
        ...
        if (simSpecialNote !== realSpecialNote) {
          ...
          const relatedTempChangeReason =
            simSpecial.tempChange?.reason || realSpecial.tempChange?.reason;
      ...
      for (const volunteer of this.volunteers) {
        ...
        const simVolTasks = simTasks
          ...
          .map((t, i) => {
            ...
            return {
              taskId: t.id,
              elderId: t.elderId,
              elderName: simElder?.name || baseElder?.name || '',
              position: pos,
            };
          })
        ...
        const realVolTasks = realTasks
          ...
          .map((t, i) => {
            ...
            return {
              taskId: t.id,
              elderId: t.elderId,
              elderName: baseElder?.name || simElder?.name || '',
              position: pos,
            };
          })
          .sort((a, b) => a.position - b.position);
        ...
        for (const eid of allElderForRoute) {
          ...
          if (oldPos !== undefined && newPos !== undefined && oldPos !== newPos) {
            const t =
              simVolTasks.find((x) => x.elderId === eid) ||
              realVolTasks.find((x) => x.elderId === eid);
            if (t) {
              movedTasks.push({
                taskId: t.taskId,
                elderId: eid,
                elderName: t.elderName,
                oldPos,
                newPos,
              });
            }
      ...
      const hasChanges =
        ...
        pausedChanges.some((p) => p.changeType !== 'pause-unchanged') ||
        specialMealChanges.length > 0;
      ...
      dayDiffs[date] = {
        ...
        assignedTasks: {
          ...
          after: afterAssigned,
          diff: afterAssigned - beforeAssigned,
        },
        addedTasks,
        removedTasks,
        volunteerChanges,
  ...
  openSimulationDiffPreview() {
    ...
    if (this.simulationDiffResult && this.simulationData) {
      this.simulationDiffDate =
        this.simulationDiffResult.summary.datesWithChanges[0] || this.simulationData.dates[0];
  ...
  locateElderInDiff(elderId: string) {
    ...
    if (elder) {
      this.showSyncToast(
        `已定位到老人：${elder.name}（${this.simulationDiffDate || '当前日期'}）`,
        'info',
      );
    }
  }
  ...
  locateVolunteerInDiff(volunteerId: string) {
    ...
    if (volunteer && this.simulationData) {
      ...
      this.showSyncToast(
        `已定位到志愿者：${volunteer.name}，请查看路线顺序页签（${this.simulationViewDate || '当前日期'}）`,
        'info',
      );
    }
  }
  ...
  getDiffDayBadgeCount(date: string): number {
    ...
    if (!day) return 0;
    return (
      day.addedTasks.length +
      day.removedTasks.length +
      day.volunteerChanges.length +
      day.routeChanges.length +
      day.specialMealChanges.length
    );
  }
  ...
  getTaskChangeVolunteerText(oldVol: string | undefined, newVol: string | undefined): string {
  ...
  confirmSubmitSimulation() {
    ...
    for (const date of this.simulationData.dates) {
      ...
      const finalDateTaskIds = new Set(this.tasks.filter((t) => t.date === date).map((t) => t.id));
      if (simDateSort) {
        for (const volId of Object.keys(simDateSort)) {
          const realIds = simDateSort[volId]
            .map((simId) => {
              ...
              return (
                existing?.id ||
                this.tasks.find((t) => t.date === simTask.date && t.elderId === simTask.elderId)
                  ?.id ||
                simId
              );
            })
      ...
      for (const volId of Object.keys(this.kanbanSort[date])) {
        this.kanbanSort[date][volId] = this.kanbanSort[date][volId].filter((id) =>
          finalDateTaskIds.has(id),
        );
      }
    ...
    for (const date of this.simulationData.dates) {
      const validTaskIds = new Set(this.tasks.filter((t) => t.date === date).map((t) => t.id));
    ...
    this.showSyncToast(
      `模拟排班已提交${detail}，正式数据、备餐/配送/闭环看板已同步更新`,
      'success',
    );
  }
  ...
  setStatus(id: string, status: MealTask['status']) {
    if (this.simulationMode === 'active' && this.simulationData) {
      this.simulationData.tasks = this.simulationData.tasks.map((task) =>
        task.id === id
          ? { ...task, status, exception: status === '异常' ? task.exception : '' }
          : task,
      );
      this.computeAllSimulationStats();
      return;
  ...
  assignTaskWithConflictCheck(taskId: string, volunteerId: string) {
    ...
    if (tempChange.volunteerId && tempChange.volunteerId !== volunteerId) {
      ...
      const newVol = volunteerId
        ? this.volunteers.find((v) => v.id === volunteerId)?.name
        : '未分配';
      conflicts.push(
        `临时变更中原本指定志愿者为「${curVol?.name || tempChange.volunteerId}」，当前选择为「${newVol}」`,
      );
    }
    if (tempChange.volunteerId && !volunteerId) {
      const curVol = this.volunteers.find((v) => v.id === tempChange.volunteerId);
      conflicts.push(
        `临时变更原本指定了志愿者「${curVol?.name || tempChange.volunteerId}」，当前选择「未分配」将取消指定`,
      );
    }
  ...
  submitException() {
    ...
    this.tasks = this.tasks.map((t) =>
      t.id === this.exceptionFormTaskId
        ? { ...t, status: '异常' as const, exception: this.exceptionForm.description }
        : t,
    );
    this.saveExceptions();
    this.save();
  ...
  updateExceptionStatus(recordId: string, newStatus: ExceptionStatus) {
    ...
    if (newStatus === '已解决') {
      ...
      if (record) {
        this.tasks = this.tasks.map((t) =>
          t.id === record.taskId && t.status === '异常'
            ? { ...t, status: '已送达' as const, exception: '' }
            : t,
        );
        this.save();
      }
  ...
  exceptionCountByStatus(status: ExceptionStatus): number {
    return this.exceptionRecords.filter((r) => r.date === this.taskDate && r.status === status)
      .length;
  }
  ...
  elderPreference(id: string, date?: string) {
    ...
    if (change?.mealTagIds) {
      const tagNames = change.mealTagIds
        .map((tid) => this.mealTags.find((t) => t.id === tid)?.name)
        .filter(Boolean)
        .join('、');
  ...
  addMealTag() {
    ...
    this.mealTags = [
      ...this.mealTags,
      { id, name: this.newTagName.trim(), color: TAG_COLORS[colorIndex] },
    ];
    this.newTagName = '';
    this.saveMealTags();
  }
  ...
  private detectTempChangeConflicts(elderId: string, date: string): string[] {
    ...
    if (task) {
      if (task.isManuallyModified) {
        conflicts.push(
          `该日期存在手动修改的任务（状态：${task.status}），临时变更将覆盖任务中的志愿者分配和特殊餐食备注`,
        );
      }
      if (task.status === '异常') {
        conflicts.push(
          `该日期存在异常状态的任务（异常信息：${task.exception || '无'}），请确认是否仍要变更`,
        );
      }
    }
    const exceptions = this.exceptionRecords.filter(
      (r) => r.elderId === elderId && r.date === date && r.status !== '已解决',
    );
    if (exceptions.length > 0) {
      conflicts.push(
        `该日期存在 ${exceptions.length} 条未解决的异常记录，变更可能影响异常处理流程`,
      );
    }
    const existingChange = this.temporaryDeliveryChanges.find(
      (c) => c.elderId === elderId && c.date === date && c.id !== this.editingTempChangeId,
    );
    if (existingChange) {
  ...
  saveTempChange() {
    if (
      !this.tempChangeFormElderId ||
      !this.tempChangeForm.date ||
      !this.tempChangeForm.reason.trim()
    )
      return;

    const conflicts = this.detectTempChangeConflicts(
      this.tempChangeFormElderId,
      this.tempChangeForm.date,
    );
  ...
  deleteTempChange(id: string, fromManagePanel: boolean = false) {
    ...
    const prepImpactTaskIds = this.mealPrepService.onTempChangeCancelled(
      change,
      elderIdToTaskIdMap,
    );
    ...
    if (fromManagePanel) {
      ...
      if (prepImpactTaskIds.length > 0) {
        this.showSyncToast(
          `临时变更已取消，已重置 ${prepImpactTaskIds.length} 条备餐状态，相关数据已同步`,
          'info',
        );
  ...
  private revertTaskFromTempChange(elderId: string, date: string) {
    ...
    this.tasks = this.tasks.map((t) => {
      ...
      if (change.volunteerId !== undefined) {
        const activeAssignmentsForElder = this.temporaryDeliveryChanges.filter(
          (c) =>
            c.elderId === elderId &&
            c.date === date &&
            c.id !== change.id &&
            c.volunteerId !== undefined,
        );
      ...
      if (change.specialMealNote !== undefined) {
        const activeNoteChanges = this.temporaryDeliveryChanges.filter(
          (c) =>
            c.elderId === elderId &&
            c.date === date &&
            c.id !== change.id &&
            c.specialMealNote !== undefined,
        );
  ...
  getTempChangeStatus(change: TemporaryDeliveryChange): 'today' | 'upcoming' | 'expired' {
    ...
    const diffDays = Math.floor(
      (changeDate.getTime() - todayDate.getTime()) / (1000 * 60 * 60 * 24),
    );
  ...
  getTempChangeStatusLabel(change: TemporaryDeliveryChange): string {
    ...
    const diffDays = Math.ceil(
      (new Date(change.date).getTime() - new Date(today).getTime()) / (1000 * 60 * 60 * 24),
    );
  ...
  cleanupExpiredTempChanges() {
    const expired = this.temporaryDeliveryChanges.filter(
      (c) => this.getTempChangeStatus(c) === 'expired',
    );
    if (expired.length === 0) {
    ...
    if (
      !confirm(
        `确认清理 ${expired.length} 条已过期的临时变更？\n这些变更日期已过，不再影响任何排班数据。`,
      )
    )
      return;

    const elderIdToTaskIdMap = this.buildElderIdDateTaskIdMap();
    const batchImpact = this.mealPrepService.onTempChangesCancelledBatch(
      expired,
      elderIdToTaskIdMap,
    );
    ...
    this.temporaryDeliveryChanges = this.temporaryDeliveryChanges.filter(
    ...
    const resetInfo =
      batchImpact.size > 0
        ? `，并重置 ${Array.from(batchImpact.values()).flat().length} 条备餐状态`
        : '';
  ...
  filteredPhoneNotifications(): PhoneNotification[] {
    ...
    if (this.phoneNotificationFilter !== '全部') {
      notifications = notifications.filter(
        (n) => n.notificationStatus === this.phoneNotificationFilter,
      );
    }
  ...
  updateNotificationStatus(
    ...
    this.phoneNotifications = this.phoneNotifications.map((n) =>
      n.id === notificationId
        ? { ...n, notificationStatus: status, remark: remark ?? n.remark, updatedAt: timeStr }
        : n,
    );
    this.savePhoneNotifications();
  }
  ...
  completeCallbackTask(taskId: string) {
    ...
    const relatedException = this.exceptionRecords.find(
      (e) => e.taskId === task.taskId && e.status !== '已解决',
    );
  ...
  updateCallbackStatus(taskId: string, status: CallbackTask['status']) {
    ...
    if (status === '已完成') {
      ...
      if (task) {
        this.phoneNotifications = this.phoneNotifications.map((n) =>
          n.id === task.notificationId
            ? { ...n, notificationStatus: '已通知', updatedAt: timeStr }
            : n,
        );
        this.savePhoneNotifications();
      }
  ...
  hasActiveCallbackTask(taskId: string): boolean {
    return this.getTaskCallbacks(taskId).some(
      (t) => t.status !== '已完成' && t.status !== '已取消',
    );
  }
  ...
  private shouldCreateAutoCallback(notification: PhoneNotification): boolean {
    return (
      notification.notificationStatus === '未接通' || notification.notificationStatus === '稍后再拨'
    );
  }
  ...
  applyAllConflicts() {
    ...
    if (this.isImportConflictResolutionMode && this.pendingImportData) {
      ...
      this.exceptionRecords = this.sync.deduplicateArray(this.exceptionRecords, (r) =>
        this.sync.buildDedupKeyForException({
          taskId: r.taskId,
          source: r.source,
          category: r.category,
        }),
      );
      this.phoneNotifications = this.sync.deduplicateArray(this.phoneNotifications, (n) =>
        this.sync.buildDedupKeyForNotification({
          taskId: n.taskId,
          source: n.source,
          targetId: n.targetId,
        }),
      );
      this.callbackTasks = this.sync.deduplicateArray(this.callbackTasks, (c) =>
        this.sync.buildDedupKeyForCallback({ notificationId: c.notificationId, status: c.status }),
      );
      this.temporaryDeliveryChanges = mergeById(
        this.temporaryDeliveryChanges,
        backup.temporaryDeliveryChanges || [],
      );
      this.save();
      this.saveKanbanSort();
      this.saveMealTags();
  ...
  private applyConflictGroup(group: SyncConflictGroup) {
    ...
    switch (dataType) {
      ...
      case 'mealTags':
        ...
        break;
      case 'exceptionRecords':
        this.exceptionRecords = this.sync.mergeConflicts(
          group,
          this.exceptionRecords,
        ) as ExceptionRecord[];
        this.saveExceptions();
      ...
      case 'visitRecords':
        ...
        break;
      case 'phoneNotifications':
        this.phoneNotifications = this.sync.mergeConflicts(
          group,
          this.phoneNotifications,
        ) as PhoneNotification[];
        this.savePhoneNotifications();
        break;
      case 'callbackTasks':
      ...
      case 'kanbanSort':
        ...
        break;
      case 'temporaryDeliveryChanges':
        this.temporaryDeliveryChanges = this.sync.mergeConflicts(
          group,
          this.temporaryDeliveryChanges,
        ) as TemporaryDeliveryChange[];
        this.saveTempChanges();
        break;
      case 'prepData':
  ...
  dismissConflictPanel() {
    if (
      this.activeConflictGroups.length > 0 &&
      !confirm('仍有未解决的冲突，忽略可能导致数据不一致。确认关闭？')
    ) {
      return;
    }
    this.conflictPanelVisible = false;
  ...
  private validateAndPreviewBackup(data: any) {
    ...
    if (!Array.isArray(data.tasks)) {
      ...
      for (let i = 0; i < data.tasks.length; i++) {
        ...
        if (
          ...
          errors.push(
            `任务[${i}]: status 值"${task.status}"无效，应为 ${VALID_STATUSES.join('/')}`,
          );
        }
      }
    }
    ...
    if (data.exceptionRecords !== undefined) {
      if (!Array.isArray(data.exceptionRecords)) {
        ...
        for (let i = 0; i < data.exceptionRecords.length; i++) {
          ...
          if (
            'category' in exc &&
            typeof exc.category === 'string' &&
            !VALID_CATEGORIES.includes(exc.category)
          ) {
            errors.push(`异常记录[${i}]: category 值"${exc.category}"无效`);
          }
          if (
            'severity' in exc &&
            typeof exc.severity === 'string' &&
            !VALID_SEVERITIES.includes(exc.severity)
          ) {
            errors.push(`异常记录[${i}]: severity 值"${exc.severity}"无效`);
          }
          if (
            'status' in exc &&
            typeof exc.status === 'string' &&
            !VALID_EXC_STATUSES.includes(exc.status)
          ) {
    ...
    if (data.visitRecords !== undefined) {
      if (!Array.isArray(data.visitRecords)) {
        ...
        for (let i = 0; i < data.visitRecords.length; i++) {
          ...
          if (
            'visitMethod' in visit &&
            typeof visit.visitMethod === 'string' &&
            !VALID_METHODS.includes(visit.visitMethod)
          ) {
    ...
    if (
      data.prepData !== undefined &&
      (!data.prepData || typeof data.prepData !== 'object' || Array.isArray(data.prepData))
    ) {
    ...
    if (
      ...
      (!data.deliveryData ||
        typeof data.deliveryData !== 'object' ||
        Array.isArray(data.deliveryData))
    ) {
      errors.push('deliveryData 字段格式不正确（必须为对象）');
    }

    if (
      data.temporaryDeliveryChanges !== undefined &&
      !Array.isArray(data.temporaryDeliveryChanges)
    ) {
    ...
    const nestedStatusCount = (
      storageData: Record<string, Record<string, unknown>> | undefined,
    ): number => {
      if (!storageData) return 0;
      return Object.values(storageData).reduce(
    ...
    const totalCount =
      backup.elders.length +
      backup.volunteers.length +
      backup.tasks.length +
      backup.mealTags.length +
      backup.exceptionRecords.length +
      backup.visitRecords.length +
      backup.phoneNotifications.length +
      backup.callbackTasks.length +
      (backup.temporaryDeliveryChanges || []).length +
      nestedStatusCount(backup.prepData) +
      nestedStatusCount(backup.deliveryData);
  ...
  private countImportItemsByStatus<T>(
  ...
  get importPreviewSummary() {
    ...
    return {
      elders: {
        total: p.elders.length,
        new: this.countImportItemsByStatus(p.elders, 'new'),
        duplicate: this.countImportItemsByStatus(p.elders, 'duplicate'),
        overwrite: this.countImportItemsByStatus(p.elders, 'overwrite'),
      },
      volunteers: {
        total: p.volunteers.length,
        new: this.countImportItemsByStatus(p.volunteers, 'new'),
        duplicate: this.countImportItemsByStatus(p.volunteers, 'duplicate'),
        overwrite: this.countImportItemsByStatus(p.volunteers, 'overwrite'),
      },
      tasks: {
        total: p.tasks.length,
        new: this.countImportItemsByStatus(p.tasks, 'new'),
        duplicate: this.countImportItemsByStatus(p.tasks, 'duplicate'),
        overwrite: this.countImportItemsByStatus(p.tasks, 'overwrite'),
      },
      mealTags: {
        total: p.mealTags.length,
        new: this.countImportItemsByStatus(p.mealTags, 'new'),
        duplicate: this.countImportItemsByStatus(p.mealTags, 'duplicate'),
        overwrite: this.countImportItemsByStatus(p.mealTags, 'overwrite'),
      },
      exceptionRecords: {
      ...
      visitRecords: {
        total: p.visitRecords.length,
        new: this.countImportItemsByStatus(p.visitRecords, 'new'),
        duplicate: this.countImportItemsByStatus(p.visitRecords, 'duplicate'),
        overwrite: this.countImportItemsByStatus(p.visitRecords, 'overwrite'),
      },
      phoneNotifications: {
      ...
      callbackTasks: {
        total: p.callbackTasks.length,
        new: this.countImportItemsByStatus(p.callbackTasks, 'new'),
        duplicate: this.countImportItemsByStatus(p.callbackTasks, 'duplicate'),
        overwrite: this.countImportItemsByStatus(p.callbackTasks, 'overwrite'),
      },
      temporaryDeliveryChanges: {
        total: p.temporaryDeliveryChanges.length,
        new: this.countImportItemsByStatus(p.temporaryDeliveryChanges, 'new'),
        duplicate: this.countImportItemsByStatus(p.temporaryDeliveryChanges, 'duplicate'),
        overwrite: this.countImportItemsByStatus(p.temporaryDeliveryChanges, 'overwrite'),
      },
    };
  }
  ...
  private isValidTask(t: any): boolean {
    return (
      ...
      typeof t === 'object' &&
      typeof t.id === 'string' &&
      typeof t.elderId === 'string' &&
      typeof t.date === 'string' &&
      typeof t.volunteerId === 'string' &&
      typeof t.status === 'string' &&
      typeof t.exception === 'string' &&
  ...
  private isValidVisitRecord(r: any): boolean {
    return (
      ...
      typeof r.id === 'string' &&
      typeof r.elderId === 'string' &&
      typeof r.visitDate === 'string' &&
      typeof r.visitMethod === 'string' &&
      typeof r.healthFeedback === 'string' &&
      typeof r.mealFeedback === 'string' &&
      typeof r.nextAttention === 'string' &&
      typeof r.createdAt === 'string'
    );
  }
  ...
  private isValidCallbackTask(c: any): boolean {
    return (
      ...
      typeof c.elderId === 'string' &&
      typeof c.date === 'string' &&
      typeof c.phone === 'string' &&
      typeof c.nextCallbackTime === 'string' &&
      typeof c.handler === 'string' &&
      typeof c.status === 'string' &&
      typeof c.result === 'string' &&
      typeof c.callbackCount === 'number' &&
  ...
  confirmImport() {
    ...
    if (
      !Array.isArray(backup.volunteers) ||
      backup.volunteers.some((v: any) => !this.isValidVolunteer(v))
    ) {
    ...
    if (
      backup.mealTags.length > 0 &&
      (!Array.isArray(backup.mealTags) || backup.mealTags.some((t: any) => !this.isValidMealTag(t)))
    ) {
      integrityErrors.push('餐食标签数据不完整，存在缺失字段的记录');
    }
    if (
      backup.exceptionRecords.length > 0 &&
      (!Array.isArray(backup.exceptionRecords) ||
        backup.exceptionRecords.some((r: any) => !this.isValidExceptionRecord(r)))
    ) {
      integrityErrors.push('异常记录数据不完整，存在缺失字段的记录');
    }
    if (
      backup.visitRecords.length > 0 &&
      (!Array.isArray(backup.visitRecords) ||
        backup.visitRecords.some((r: any) => !this.isValidVisitRecord(r)))
    ) {
      integrityErrors.push('回访记录数据不完整，存在缺失字段的记录');
    }
    if (
      backup.phoneNotifications.length > 0 &&
      (!Array.isArray(backup.phoneNotifications) ||
        backup.phoneNotifications.some((n: any) => !this.isValidPhoneNotification(n)))
    ) {
      integrityErrors.push('电话通知数据不完整，存在缺失字段的记录');
    }
    if (
      backup.callbackTasks.length > 0 &&
      (!Array.isArray(backup.callbackTasks) ||
        backup.callbackTasks.some((c: any) => !this.isValidCallbackTask(c)))
    ) {
    ...
    if (backup.deliveryData) {
      detectionPairs.push([
        'deliveryData',
        backup.deliveryData,
        this.volunteerDeliveryService.exportStorageData(),
      ]);
    }
    ...
    if (conflictGroups.length > 0) {
      ...
      this.showSyncToast(
        `导入数据检测到 ${conflictGroups.reduce((s, g) => s + g.conflicts.length, 0)} 个冲突，请处理`,
        'warn',
      );
      return;
    }
